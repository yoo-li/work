	...
	<div class="mui-inner-wrap">
		...
		<nav class="mui-bar mui-bar-tab" style="height:40px;line-height:40px;">
			<div class="mui-tab-item" style="width:60%;color:#929292;">
				...
				<span class="mui-tab-label">
					...
					<div class="mui-pull-right price-info">
						<p>商品金额:<span class="price" id="total_moneyedit">¥{$total_moneyedit}</span>元&nbsp;</p>
						共&nbsp;<span class="total_quantity">{$total_quantity}</span>&nbsp;件<!--（邮费<span style="display: none" id="postage">
			...
			<a class="mui-tab-item confirmpayment" href="#" style="width:30%">
				<span class="mui-icon iconfont icon-feiyongshuomingicon button-color">&nbsp;结算(<span class="total_quantity">{$total_quantity}</span>)</span>
			</a>
		</nav>
						...
						<ul id="shoppingcart_wrap" class="mui-table-view mui-table-view-chevron" style="color: #333;">
							{foreach name="shoppingcarts" item=shoppingcart_info  from=$shoppingcarts}
								<li class="mui-input-row mui-checkbox mui-left" style="min-height:100px;height: auto;"
									...
									<label>
										<img class="mui-media-object mui-pull-left goods-img" data-id="{$shoppingcart_info.id}" src="{$shoppingcart_info.productthumbnail}">
										<div class="mui-media-body">
											<p class='goods-title'>{$shoppingcart_info.productname}</p>
											{if $shoppingcart_info.vendorname neq '' && $supplier_info.showvendor eq '1'}
											<p class='mui-ellipsis'>供应商：{$shoppingcart_info.vendorname}</p>
											{/if}
											<p class='mui-ellipsis vendorname' style="display: none;">供应商：{$shoppingcart_info.vendorname}</p>
											{if $shoppingcart_info.propertydesc neq ''}
												<p class='mui-ellipsis'>属性：{$shoppingcart_info.propertydesc}</p>
											{/if}
											<div class='mui-ellipsis'>
												<div class="mui-numbox" data-numbox-step='1' data-numbox-min='1' data-numbox-max='{if $shoppingcart_info.activitymode eq "1"}1{else}99{/if}'>
													<button class="mui-btn mui-numbox-btn-minus" type="button" data-id="{$shoppingcart_info.id}">-</button>
													<input class="productid" value="{$shoppingcart_info.productid}" id="productid_{$shoppingcart_info.id}" type="hidden"/>
											...
											<p class='mui-ellipsis unit-price'>{if $shoppingcart_info.zhekou neq ''}{if $shoppingcart_info.activitymode eq '1'}底价{else}活动价{/if}：
											...
											<div class='mui-ellipsis total-price'>
                                                {*<span id="total_price_{$shoppingcart_info.id}"class="price">¥{$shoppingcart_info.total_price}</span>*}
									...
									<input name="shoppingcart[]" value="{$shoppingcart_info.id}" id="shoppingcart_{$shoppingcart_info.id}"
										   checked type="checkbox" class="select-check">
								</li>
								{foreachelse}
...
<script>
	...
	mui.ready(function ()
			  {
				  ...
				  mui('.mui-bar').on('tap', 'a.confirmpayment', function (e)
				  {
					  ...
					  {
						  var vendornames=mui(".vendorname");
							 var selectchecks=mui(".select-check");
							 var products=mui(".productid");
							 var selectchecksids=[];
							 var productids='';
						  ...
						  for(var i=0;i<vendornames.length;i++){
							  if(selectchecks[i].checked){
								  selectchecksids[selectchecksids.length]=selectchecks[i].value;
								    productids+=products[i].value+',';
								  vendorname=vendornames[i].innerHTML;
							  	  if(vendorname.substring(vendorname.indexOf("：")+1)=="jdvop"){
									  cansubmit1=1;
							 	  }else{
									  cansubmit2=1;
								  }
						...
						//   console.log(productids);
						  ...
						  if(cansubmit1&&cansubmit2){
							  mui.toast("京东与非京东商品,请分开下单!");
						  }else{
					  mui.ajax({
								 type: 'POST',
								 async: false,
								 url: "shoppingcart_smk.ajax.php",
								 data: 'selectchecksids=' + selectchecksids +'&productid=' + productids,
								 success: function (json)
								 {
									   if(json == 201){
										   mui.toast("您只能选择一类活动商品进行购买！");
										   return;
									   }else if (json == 202) {
										   mui.toast("不同自提商家商品,请分开下单!");
											  return;
									  }else{
										  document.frm.submit();
	...
	function recalc()
	{
		...
		total_moneyedit = total_money - allpostage;
		Zepto("#total_money").html('¥' + total_money.toFixed(2));
		Zepto("#total_moneyedit").html('¥' + total_moneyedit.toFixed(2));
		Zepto(".total_quantity").html(total_quantity);
        Zepto("#postage").html(allpostage);
