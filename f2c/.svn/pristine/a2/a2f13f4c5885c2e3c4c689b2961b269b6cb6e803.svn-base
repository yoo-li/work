...
$mall_vipcards = XN_Query::create ( 'Content' )->tag('mall_vipcards')
	...
	$mall_usages = XN_Query::create ( 'YearContent' )->tag('mall_usages_'.$profileid)
		...
		->end(-1)
		->execute ();
$smarty->assign("usageslistscount",count($mall_usages));
...
$SmkUsersss = XN_Query::create ( 'Content' )->tag('Mall_SmkUsers')
    ...
    ->end(1)
    ->execute();
$SmkUsersss[0]->my->totle_account==''|| number_format($SmkUsersss[0]->my->totle_account,2)==0?$totle_account=0:$totle_account=$SmkUsersss[0]->my->totle_account;
$smarty->assign('totle_account',$totle_account);
