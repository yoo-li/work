...
require_once (dirname(__FILE__) . "/pcpayment.func.php");
		...
		{  
			...
			else
			{
				...
				if (count($profile_info) > 0)
				{
					...
					if (round(floatval($needpayable)+$availablenumber+$total_discount,2) == round(floatval($sumorderstotal),2))  
					{ 
						...
						{
							$jsApiParameters = weixin_pc_jsapi($profileid,$ordername,'1',$order_no,$orderid);
						}
						else
						{
							$jsApiParameters = weixin_pc_jsapi($profileid,$ordername,$amount,$order_no,$orderid);
						} 
