	...
	if (count($supplier_profile) > 0)
	{
		...
		if (count($mall_officialauthorizeevents) > 0)
		{
			...
			if (count($mall_officialenterprisecurrencysauthorizes) > 0)
			{
				...
				{
                    official($order_info,$mall_officialenterprisecurrencysauthorize_info,$mall_officialauthorizeevent_info,$amount);
...
//生成事务官审批记录
function official($order_info,$mall_officialenterprisecurrencysauthorize_info,$mall_officialauthorizeevent_info,$amount)
{
    try{
        ...
        if ($copyrights['official'] == '1')
        {
            ...
            if (count($supplier_profile) > 0)
            {
                ...
                $newcontent->my->mall_officialordersstatus = 'Agree';
...
function official_notify($order_info,$amount)
{ 
    try
    {  
