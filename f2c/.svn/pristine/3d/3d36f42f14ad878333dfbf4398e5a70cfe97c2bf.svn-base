        ...
        <div class="mui-content mysure">
            <form method="post" name="frm" action="/confirmpayment.php">
                ...
                <ul class="mui-table-view">
                    ...
                    {if $address_list neq ''}
                    <input id="address_list" type="hidden" value="{$address_list}">
                    <li class="mui-table-view-cell order_lift">
                        <p>自提时间和地址</p>
                        <form class="mui-input-group">
                            <div class="mui-input-row">
                                <div id="lift_date" class="mui-table-view-cell order_lift_choose">
                                    <label>提取日期 :</label>
                                    <input type="text" class="mui-input-clear lift_date" readonly name="date">
                                </div>
                                <div id="lift_time" class="mui-table-view-cell order_lift_choose">
                                    <label>提取时间 :</label>
                                    <input type="text" class="mui-input-clear lift_time" readonly name="time">
                                </div>
                                <div id="order_lift_adress" class="mui-table-view-cell order_lift_choose">
                                    <label>自提门店地址 :</label>
                                    <input type="text" class="mui-input-clear order_lift_adress" readonly name="adress">
...
//        mui('#pullrefresh').scroll();
        ...
        mui('.mui-bar').on('tap', 'button.confirmpayment', function (e)
        {
            ...
            var address_list=$('#address_list').val();
            var lift_date=$('.lift_date').val();
            var lift_time=$('.lift_time').val();
            var lift_adress=$('.order_lift_adress').val();
            if (deliveraddress_count != "0")
            {
                if(!address_list){
                    document.frm.submit();
                }else{
                    if(lift_date&&lift_time&&lift_adress){
                        document.frm.submit();
                    }else{
                        sweetAlert("警告", "请先完善自提信息，谢谢！", "error");
                    }
                }
            }
            else
