...
{
	messagebox('提示','请从微信公众号“特赞商城”或朋友圈中朋友分享链接进入本平台，如您确实采用上述方式仍然出现本信息，请与系统管理员联系。');
	die();
}
...
try{
    ...
    $orders_products = XN_Query::create ( 'YearContent' )->tag('mall_orders_products_'.$profileid )
        ...
        ->end(-1)
        ->execute ();
    $order_vendorid=$order_info->my->vendorid;//当前订单所选供货商

	$cur_vendorid=-1;//当前登陆用户对应供货商
	$vendoraddress=XN_Query::create ( 'Content' )
		->tag('mall_vendorsaddress' )
        ->filter ( 'type', 'eic', 'mall_vendorsaddress' )
        ->filter ( 'my.supplierid', '=',$supplierid)
		->filter ( 'my.wx_profileid','=',$profileid)
        ->filter ( 'my.deleted', '=','0')
        ->end(1)
        ->execute ();
	if(count($vendoraddress)){
        $cur_vendorid=$vendoraddress[0]->my->vendorid;
	}
	if($order_vendorid-$cur_vendorid>0 || $order_vendorid-$cur_vendorid<0){
        messagebox('提示','此订单所选供货商不是本店，请提醒用户去：<p style="color:red;font-size: 18px;text-align: center;">'.$order_info->my->smkadress.'</p>提货，如有疑问，请与系统管理员联系。');
        die();
	}
	...
	if (isset($deliveryid) && $deliveryid != "")
	{
        ...
        // foreach ($mall_settlementorders as $k=>$v){
        //
        //     $orderinfo['deliverytime'][$k] = $v->my->deliverytime;
        //     $delivery = XN_Content::load($v->my->delivery,"logistics");
        //     $orderinfo['deliveryname'][$k] = $delivery->my->logisticsname;
        //     $orderinfo['invoicenumber'][$k] = $v->my->invoicenumber;
        //
        //     if ($orderinfo['deliveryname'][$k] == "京东")
        //     {
        //         require_once(dirname(__FILE__) . "/jd.func.php");
        //         $jdOrderId = $order_info->my->invoicenumber;
        //         $jdorder_info = JD::getjdorderinfo($jdOrderId);
        //         $cOrders = $jdorder_info['cOrder'];
        //         if (count($cOrders) > 0)
        //         {
        //             $invoicenumbers = array();
        //             foreach($cOrders as $cOrder_info)
        //             {
        //                 $invoicenumbers[] = $cOrder_info['jdOrderId'];
        //             }
        //             $orderinfo['invoicenumber'] = "JD";
        //             $orderinfo['invoicenumbers'] = $invoicenumbers;
        //         }
        //     }
        // }

		$orderinfo['invoicenumber'][0] = $order_info->my->invoicenumber; //发货单号
		$orderinfo['delivery'][0] = $order_info->my->delivery;
		$orderinfo['deliverytime'][0] = $order_info->my->deliverytime; //发货时间
		$loadcontent = XN_Content::load($order_info->my->delivery,"logistics");
		$orderinfo['deliveryname'][0] = $loadcontent->my->logisticsname; //物流公司

		if ($orderinfo['deliveryname'][0] == "京东")
		{
			require_once(dirname(__FILE__) . "/jd.func.php");
		    $jdOrderId = $order_info->my->invoicenumber;
		    $jdorder_info = JD::getjdorderinfo($jdOrderId);
