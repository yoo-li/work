...
<div class="mui-off-canvas-wrap mui-draggable">
	<div class="mui-inner-wrap">
		...
		<header class="mui-bar mui-bar-nav mui-row myorder_header">
			...
			<div class="mui-bar mui-title" style="margin-top: 40px;right: 0;left: 0;">
				<div id="segmentedControl" class="myorder_list_hd mui-segmented-control mui-segmented-control-inverted mui-segmented-control-negative">
					<a href="smk_orders_pendingpayment.php" class="mui-control-item mui-active">待付款</a>
					<a href="smk_orders_sendout.php" class="mui-control-item">待发货</a>
					<a href="smk_orders_receipt.php" class="mui-control-item">待收货</a>
					<a href="smk_orders_appraise.php" class="mui-control-item">待评价</a>
					<a href="smk_orders_aftersaleservice.php" class="mui-control-item">售后服务</a>
				</div>
			</div>
		</header>
...
<script type="text/javascript" charset="utf-8">
	...
	mui.ready(function ()
	{
		...
		mui('.mui-table-view').on('tap', 'button.deleteorder', function (e)
		{
			...
			{
				window.location.href = 'smk_orders_pendingpayment.php?type=delete&record=' + orderid;
			});
		});
		mui('.mui-table-view').on('tap', 'button.submitorder', function (e)
		{
			var orderid          = this.getAttribute('data-id');
			window.location.href = 'submitorder.php?record=' + orderid;
		});
	});
	...
	function order_html(v)
	{
		...
		sb.append('<p>订单号:<span>'+ v.mall_orders_no +'</span></p>');
		sb.append('</div>');
		...
		sb.append('<button type="button" class="mui-btn deleteorder " data-id="' + v.orderid + '">取消订单</button>');
		sb.append('<button type="button" class="mui-btn red submitorder" data-id="' + v.orderid + '">付款</button>');
		sb.append('</div>');
		sb.append('</li>');
		return sb.toString();
	}
	function order_empty_html()
	{
		...
		sb.append('					      <p class="msgbody">您的货到付款订单还是空的，快去选购商品吧<br>');
