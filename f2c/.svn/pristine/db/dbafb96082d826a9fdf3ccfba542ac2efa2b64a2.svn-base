<?php
	...
	try
	{
		...
		if(isset($token) && !empty($token))
		{
            ...
            $allmergepostage=$robprice>=119?0:$productContent->my->postage;
            $alltotle_money = number_format(floatval($total_money) + floatval($addpostage) + floatval($allmergepostage), 2, ".", "");
            ...
            $newcontent->my->postage       = number_format(floatval($addpostage) + floatval($allmergepostage), 2, ".", "");
            ...
            $orders_product->my->postage      = number_format($allmergepostage, 2, ".", "");
            $orders_product->my->includepost  = $allmergepostage;
            $orders_product->my->mergepostage = $allmergepostage;
