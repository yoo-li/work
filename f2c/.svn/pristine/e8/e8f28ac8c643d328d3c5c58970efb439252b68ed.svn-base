...
try {
	...
	$orders = XN_Query::create('YearContent')->tag('mall_orders')
       ...
       $mall_settings = XN_Query::create('Content')->tag('mall_settings')
	    ...
	    //秒杀商品超过半小时未支付，自动取消资格，返还到可抢数量中
        $robproducts=XN_Query::create("Content")
            ->tag("mall_robproducts")
            ->filter("type","eic","mall_robproducts")
            ->filter('published','>=',date('Y-m-d H:i:s',strtotime("-1 hour")))
            ->filter('published','<=',date('Y-m-d H:i:s',strtotime("-30 minutes")))
            ->filter('my.robpaymentstatus','=','未支付')
            ->filter("my.deleted",'=','0')
            ->end(200)
            ->execute();
	    $add_product_numbers=array();
        if(count($robproducts)){
            foreach($robproducts as $robproduct_info){
                $activityDetailContent=XN_Content::load($robproduct_info->my->activity_product_id,"mall_robsingles_details");
                $activitynumber=$activityDetailContent->my->activitynumber;
                $newActivitynumber=$activitynumber+1;
                $activityDetailContent->my->activitynumber=$newActivitynumber;
                $activityDetailContent->save("mall_robsingles_details");

                $starttime = $activityDetailContent->my->begindate;
                $endtime = $activityDetailContent->my->enddate;
                XN_MemCache::put($activityDetailContent->my->productid.'_'.$newActivitynumber.'_'.$starttime.'_'.$endtime,$robproduct_info->my->activity_product_id);
            }
            XN_Content::delete($robproducts,"mall_robproducts");
        }
