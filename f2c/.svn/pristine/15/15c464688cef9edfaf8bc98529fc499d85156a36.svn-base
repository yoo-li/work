...
$smarty->assign("begindate",$starttime);
$smarty->assign("enddate",$endtime);
...
//    通过商品ID 取得图片
    $productthumbnail = XN_Query::create(content)
        ->tag('mall_products')
        ->filter('type','eic','mall_products')
        ->filter('id','=',$val->my->productid)
        ->execute();
    $productthumbnails = $productthumbnail[0]->my->productlogo;
    $list[$key]['productthumbnail'] = $productthumbnails;
 }
 //秒杀活动id
$smarty->assign("list_info",$list);
$smarty->assign("begindate",$starttime);
$smarty->assign("enddate",$endtime);
$smarty->assign("list_infos",$list);
