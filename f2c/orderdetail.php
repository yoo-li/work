...
$mall_smkconsumelogs = XN_Query::create('MainContent')->tag('mall_smkconsumelogs')//Mall_SmkConsumeLogs
->filter('type', 'eic', 'mall_smkconsumelogs')
    ->filter('my.orderid', '=', $orderid)
    ->end(1)
    ->execute();
if(count($mall_smkconsumelogs)){
	$amount=round($mall_smkconsumelogs[0]->my->amount,2);
}
else{
    $amount=0;
}
