...
//$_SESSION['profileid'] = '4eq2yr6x03a'; //小丁
...
{
    $smarty->assign("profile_info", get_profile_info($profileid));
}
...
//============================================================================
$list = array();
$Mall_RobSingles = XN_Query::create ( 'MainContent' )->tag ( 'Mall_RobSingles' )
    ->filter ( 'type', 'eic', 'Mall_RobSingles' )
    ->filter ( 'my.supplierid', '=', $supplierid )
    ->filter ( 'my.begindate','<=',date("Y-m-d H:i:s"))
    ->filter ( 'my.enddate','>=',date("Y-m-d H:i:s"))
    ->order('published',XN_Order::DESC)
    ->end(1)
    ->execute();

$list['begindate']= $Mall_RobSingles[0]->my->begindate;
$list['enddate']= $Mall_RobSingles[0]->my->enddate;

$Mall_RobSingles_details = XN_Query::create ( 'MainContent' )
    ->tag ( 'Mall_RobSingles_details' )
    ->filter ( 'type', 'eic', 'Mall_RobSingles_details' )
    ->filter ( 'my.salesactivityid', '=', $Mall_RobSingles[0]->id )
    ->order('id',XN_Order::ASC)
    ->execute();

foreach($Mall_RobSingles_details as $k=>$v){
    if($k == 6){break;}
    $list_Rob[$k]['productname'] = $v->my->productname;
    $list_Rob[$k]['productid'] = $v->my->productid ;
    $list_Rob[$k]['robprice'] = $v->my->robprice ;
    $list_Rob[$k]['salesactivityid'] = $v->my->salesactivityid ;

    $Mall_Products = XN_Query::create ( 'Content' )
        ->tag ( 'Mall_Products' )
        ->filter ( 'type', 'eic', 'Mall_Products' )
        ->filter ( 'id', '=', $list_Rob[$k]['productid'])
        ->end(1)
        ->execute();
    $list_Rob[$k]['productthumbnail'] = $Mall_Products[0]->my->productthumbnail;
    $list_Rob[$k]['shop_price'] = $Mall_Products[0]->my->shop_price;
}
$list['Rob'] = $list_Rob;
...
//============================================================================
$ads_list=array(
    'lunbotu1'=>'轮播图1','lunbotu2'=>'轮播图2','lunbotu3'=>'轮播图3',
    'tubiao1'=>'图标1','tubiao2'=>'图标2','tubiao3'=>'图标3','tubiao4'=>'图标4','tubiao5'=>'图标5','tubiao6'=>'图标6','tubiao7'=>'图标7','tubiao8'=>'图标8',
    'xianshizhekou'=>'限时折扣','tegongzhuanqu'=>'特供专区','remaibaokuan'=>'热卖爆款',
    'zhutitemai1'=>'主题特卖1',
    'zhutitemaishangpin11'=>'主题特卖商品1-1','zhutitemaishangpin12'=>'主题特卖商品1-2','zhutitemaishangpin13'=>'主题特卖商品1-3','zhutitemaishangpin14'=>'主题特卖商品1-4','zhutitemaishangpin15'=>'主题特卖商品1-5','zhutitemaishangpin16'=>'主题特卖商品1-6',
    'zhutitemai2'=>'主题特卖2',
    'zhutitemaishangpin21'=>'主题特卖商品2-1','zhutitemaishangpin22'=>'主题特卖商品2-2','zhutitemaishangpin23'=>'主题特卖商品2-3','zhutitemaishangpin24'=>'主题特卖商品2-4','zhutitemaishangpin25'=>'主题特卖商品2-5','zhutitemaishangpin26'=>'主题特卖商品2-6',
    'zhutitemai3'=>'主题特卖3',
    'zhutitemaishangpin31'=>'主题特卖商品3-1','zhutitemaishangpin32'=>'主题特卖商品3-2','zhutitemaishangpin33'=>'主题特卖商品3-3','zhutitemaishangpin34'=>'主题特卖商品3-4','zhutitemaishangpin35'=>'主题特卖商品3-5','zhutitemaishangpin36'=>'主题特卖商品3-6',
    'louceng1'=>'楼层1',
    'loucengshangpin11'=>'楼层商品1-1','loucengshangpin12'=>'楼层商品1-2','loucengshangpin13'=>'楼层商品1-3','loucengshangpin14'=>'楼层商品1-4',
    'louceng2'=>'楼层2',
    'loucengshangpin21'=>'楼层商品2-1','loucengshangpin22'=>'楼层商品2-2','loucengshangpin23'=>'楼层商品2-3','loucengshangpin24'=>'楼层商品2-4',
    'louceng3'=>'楼层3',
    'loucengshangpin31'=>'楼层商品3-1','loucengshangpin32'=>'楼层商品3-2','loucengshangpin33'=>'楼层商品3-3','loucengshangpin34'=>'楼层商品3-4',
    'louceng4'=>'楼层4',
    'loucengshangpin41'=>'楼层商品4-1','loucengshangpin42'=>'楼层商品4-2','loucengshangpin43'=>'楼层商品4-3','loucengshangpin44'=>'楼层商品4-4',
);

$zhutitemaishangpin1 ='[zhutitemaishangpin1]';
$zhutitemaishangpin2 ='[zhutitemaishangpin2]';
$zhutitemaishangpin3 ='[zhutitemaishangpin3]';

$ads_list_flip = array_flip($ads_list);
...
->filter('my.supplierid', '=', $supplierid)
->filter('my.status', '=', '0')
->filter('my.adname', 'in', $ads_list)
->filter('my.mall_adsstatus', '=', 'Agree')
->execute();

$ads_date = array();
foreach($ads as $k=>$v){
    $adname =  $ads_list_flip[$v->my->adname];
    $ads_date[$adname]['adname'] = $v->my->adname;
    $ads_date[$adname]['image'] = $v->my->image;
    $ads_date[$adname]['salesactivityid'] = $v->my->salesactivityid;
    $ads_date[$adname]['productid'] = $v->my->productid;
    $ads_date[$adname]['f2cadlinktarget'] = $v->my->f2cadlinktarget;
    if($ads_date[$adname]['f2cadlinktarget'] == 0){
        $Mall_Products = XN_Query::create('Content')
        ->tag('Mall_Products')
        ->filter('type', 'eic', 'Mall_Products')
        ->filter('id', '=', $ads_date[$adname]['productid'])
        ->filter('my.deleted', '=', '0')
        ->end(1)
        ->execute();
        $ads_date[$adname]['productname'] = $Mall_Products[0]->my->productname;
        $ads_date[$adname]['url'] = 'detail.php?from=index&productid='.$Mall_Products[0]->id;
        $ads_date[$adname]['productlogo'] = $Mall_Products[0]->my->productlogo;
        $ads_date[$adname]['market_price'] = $Mall_Products[0]->my->market_price;
        $ads_date[$adname]['shop_price'] = $Mall_Products[0]->my->shop_price;
    }
    if($ads_date[$adname]['f2cadlinktarget'] == 1){
        $ads_date[$adname]['url'] = 'salesactivity.php?record='.$ads_date[$adname]['salesactivityid'];
    }
    if($ads_date[$adname]['f2cadlinktarget'] == 3){
        $ads_date[$adname]['url'] = $v->my->otherlink;
    }
}
foreach($ads as $k=>$v){
    $adname =  $ads_list_flip[$v->my->adname];
    if(preg_match($zhutitemaishangpin1,$adname)){
        $salesactivityid = $ads_date['zhutitemai1']['salesactivityid'];
        $mall_salesactivitys_products = XN_Query::create ( 'Content' )->tag('mall_salesactivitys_products')
            ->filter ( 'type', 'eic', 'mall_salesactivitys_products')
            ->filter('my.deleted', '=', '0')
            ->filter ( 'my.supplierid', '=', $supplierid)
            ->filter ( 'my.salesactivityid ', '=', $salesactivityid)
            ->filter ( 'my.productid', '=', $ads_date[$adname]['productid'] )
            ->end(1)
            ->execute();
        $ads_date[$adname]['url'] = 'detail.php?from=salesactivity&productid='.$ads_date[$adname]['productid'].'&salesactivityid='.$mall_salesactivitys_products[0]->my->salesactivityid;
        $ads_date[$adname]['zhekou'] = $mall_salesactivitys_products[0]->my->zhekou;;
        $ads_date[$adname]['promotionalprice']=number_format( floatval($ads_date[$adname]['shop_price'] * ($mall_salesactivitys_products[0]->my->zhekou) / 10), 2, ".", "");
    }elseif (preg_match($zhutitemaishangpin2,$adname)) {
        $salesactivityid = $ads_date['zhutitemai2']['salesactivityid'];
        $mall_salesactivitys_products = XN_Query::create ( 'Content' )->tag('mall_salesactivitys_products')
            ->filter ( 'type', 'eic', 'mall_salesactivitys_products')
            ->filter('my.deleted', '=', '0')
            ->filter ( 'my.supplierid', '=', $supplierid)
        ...
        $ads_date[$adname]['zhekou'] = $mall_salesactivitys_products[0]->my->zhekou;;
        $ads_date[$adname]['promotionalprice']=number_format( floatval($ads_date[$adname]['shop_price'] * ($mall_salesactivitys_products[0]->my->zhekou) / 10), 2, ".", "");
    }elseif (preg_match($zhutitemaishangpin3,$adname)) {
        $salesactivityid = $ads_date['zhutitemai3']['salesactivityid'];
        $mall_salesactivitys_products = XN_Query::create ( 'Content' )->tag('mall_salesactivitys_products')
            ->filter ( 'type', 'eic', 'mall_salesactivitys_products')
            ->filter('my.deleted', '=', '0')
            ->filter ( 'my.supplierid', '=', $supplierid)
...
// var_dump($ads_date);die;
$smarty->assign('ads_date',$ads_date);

$smarty->display('smk.tpl');
