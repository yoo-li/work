<?php
	...
	try
	{
		...
		if (isset($_REQUEST['record']) && $_REQUEST['record'] != '')
		{
			...
			{
				$productid = $orders_product_info->my->productid;
                $orders_product_info->my->robproductid='';
                $orders_product_info->my->robsingledetailid='';
					...
					if ($jd == "1")
					{
						...
						if (isset($salesactivityid) && $salesactivityid != "" && $salesactivityid != "0")
						{
							...
							elseif (isset($salesactivitys_product_id) && $salesactivitys_product_id != "" && $salesactivitys_product_id != "0")
							{
								...
								if ($zhekou != $shop_zhekou || $shop_quantity != intval($quantity) || $shop_price != $new_price)
								{
			...
			foreach ($orders_products as $orders_product_info)
			{
				   ...
				   (intval($supplier_info['totalquantityfreeshipping']) <= 0 || intval($supplier_info['totalquantityfreeshipping']) > $totalquantityfreeshipping))
				{
					...
					if ($includepost <= 0 || $includepost > $productallcount)
					{
						...
						{
							$tmppostage = $shop_postage * intval($quantity);
				...
				$verify_total_quantity += intval($quantity);
		...
		elseif(isset($token) && !empty($token))
		{
			...
			foreach ($shoppingcarts as $shoppingcart_info)
			{
				...
				$productname           = $shoppingcart_info->my->productname;
				$productthumbnail      = $shoppingcart_info->my->productthumbnail;
				$quantity              = intval($shoppingcart_info->my->quantity);
				$propertydesc          = $shoppingcart_info->my->propertydesc;
				$product_property_id   = $shoppingcart_info->my->product_property_id;
				$zhekou                = $shoppingcart_info->my->zhekou;
				$zhekoulabel           = $shoppingcart_info->my->label;
	...
	if ($supplierid == '12352'&& $vendorname == 'jdvop')
	{
		...
		$stocks= JD::get_addrees_stocks($skuids,$provinceid,$cityid,$districtid);
		 
		foreach($stocks as $skuid => $stock_info)
		{
			if ($stock_info['state'] != "33")
			{
				foreach ($shoppingcart as $shoppingcart_info)
				{
					if ($skuid == $shoppingcart_info['skuid'])
					{
						$productname = $shoppingcart_info['productname'];
						$errormsg = '商品['.$productname.']在您的收货地址无货,不能购买.';
...
if($supplierid ==  '12352'){
	if($shoppingcarts){$list = $shoppingcarts;}
	if($orders_products){$list = $orders_products;}
	$vendorid = null;
	foreach($list as $v){
	...
	if($vendorid){
		...
		->filter('my.deleted', '=', '0')
		->filter('my.vendorid', $in ,$vendorid)
		->end(-1)
		->execute();

		foreach($Mall_VendorsAddress as $k=>$v){
			$address_list[$k]['address'] = $v->my->address;
