...
{
    header("Location: smk_orders_pendingpayment.php");
    die();
}
...
if(isset($_REQUEST['record']) && $_REQUEST['record'] !='' && $_REQUEST['type'] == 'delete')
{
	...
	try{
		$order_info = XN_Content::load($record,'mall_orders_'.$profileid,7);
		if ($order_info->my->tradestatus == 'trade')
		{
			messagebox('提示','已经成交的订单，不能删除。');
			die();
		}
		else if ($order_info->my->tradestatus == 'pretrade')
		{
			messagebox('提示','货到付款的订单，不能删除。');
			die();
		}
		else
		{
			$order_info->my->deleted = '1';
