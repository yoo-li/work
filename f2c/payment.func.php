...
[2015-07-14 19:04:44][debug] query:{"appid":"wx7962fafc7ec5b6c6","attach":"26122","bank_type":"CFT","cash_fee":"1","fee_type":"CNY","is_subscribe":"Y","mch_id":"1252726901","nonce_str":"50fn6p0MwXR7Ez0l","openid":"oaCu_s1UUUCiUx-plwiykseHSuj0","out_trade_no":"ORD150714030","result_code":"SUCCESS","return_code":"SUCCESS","return_msg":"OK","sign":"0C2E8D2C0EAA304529AFCC8D89041790","time_end":"20150714190421","total_fee":"1","trade_state":"SUCCESS","trade_type":"JSAPI","transaction_id":"1008040813201507140405777740"}	*/
    ...
    try
    {
        ...
        if ($tradestatus != "trade")
        {
            ...
            if ($smk_use == '1')
            {
                $use_totle_money = $sumorderstotal - $usemoney - $discount - $paymentamount;
                ...
                foreach($mall_smkcardrecords as $mall_smkcardrecord_info)
                {
                    ...
                    {
                        $smkcardrecord=$mall_smkcardrecord_info->id;
						$smkcardname=$mall_smkcardrecord_info->my->card;
                ...
                */
                ...
                $newcontent->my->profileid = $profileid;
                $newcontent->my->orderid = $orderid;
				$newcontent->my->smkcardname = $smkcardname;
...
function finished_yue_trade($order_info, $vipcardusageid = "", $vipcardusageamount = "")
{
    try
    {
        ...
        if ($tradestatus != "trade")
        {
            ...
            if (round(floatval($money) + $totle_money, 2) >= round((floatval($sumorderstotal) - $total_discount), 2))
            {
	             
	            if ($smk['smk_use'] == '1')
	            {
		             if (round($totle_money, 2) >= round(floatval($sumorderstotal) - $total_discount, 2))
		             {
			             $usemoney = 0;
			             $use_totle_money = round(floatval($sumorderstotal) - $total_discount, 2);
		             }
		             else
		             {
			             $use_totle_money = $totle_money;
			             $usemoney =  round(floatval($sumorderstotal) - $total_discount, 2) - $use_totle_money;
		             } 
	            }
	            else
	            {
                ...
                XN_MemCache::delete("mall_badges_" . $supplierid . "_" . $profileid);
                 ...
                 if ($smk['smk_use'] == '1' && $use_totle_money > 0)
                 {
	                    $mall_smkuser_info = $smk['smkuser_info']; 
		                $old_totle_money = $mall_smkuser_info->my->totle_account;
		                $totle_consumption = $mall_smkuser_info->my->totle_consumption;
		                $new_totle_money = floatval($old_totle_money) - $use_totle_money;
		                $new_totle_consumption = floatval($totle_consumption) + $use_totle_money;
...
function confirmrob($orderid){
    $orders_products = XN_Query::create('YearContent')->tag('mall_orders_products')
        ...
        ->execute();
    foreach($orders_products as $orders_product_info){
        if($orders_product_info->my->robproductid>0){
            $robproductid=$orders_product_info->my->robproductid;
            $robproductContent=XN_Content::load($robproductid,"mall_robproducts");
            $robproductContent->my->robpaymentstatus='已支付';
            $robproductContent->my->paymenttime=date("Y-m-d H:i:s");
            $robproductContent->save("mall_robproducts");
        }
    }
}
?>