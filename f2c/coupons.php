...
try{
	...
	$mall_vipcards = XN_Query::create ( 'Content' )->tag('mall_vipcards')
				...
				->execute ();
//增加兑换商品的券
    $mall_vipcards_cardtype3 = XN_Query::create ( 'Content' )->tag('mall_vipcards')
		->filter ( 'type', 'eic', 'mall_vipcards')
		->filter ( 'my.deleted', '=', '0')
		->filter ( 'my.supplierid', '=', $supplierid)
		->filter ( 'my.approvalstatus', '=', '2')
		...
		->end(-1)
		->execute ();
        $vipcardids_cardtype3 = array();
		foreach($mall_vipcards_cardtype3 as $mall_vipcard_info_cardtype3)
		{
			$vipcardids_cardtype3[] = $mall_vipcard_info_cardtype3->id;
		}
        $mall_usages_cardtype3 = XN_Query::create ( 'YearContent' )->tag('mall_usages_'.$profileid)
			->filter ( 'type', 'eic', 'mall_usages')
			->filter ( 'my.deleted', '=', '0')
			->filter ( 'my.supplierid', '=', $supplierid)
			->filter ( 'my.profileid', '=', $profileid)
            ->filter ( 'my.vipcardid', 'in', $vipcardids_cardtype3)
			->filter ( 'my.mall_usagesstatus', '!=', '已使用')
			->end(-1)
			->execute ();

        foreach($mall_usages_cardtype3 as $mall_usage_info_cardtype3)
        {
            $vipcardid = $mall_usage_info_cardtype3->my->vipcardid;
            $ids[$vipcardid] += 1;
            $usageslists_cardtype3[$vipcardid] = $mall_usage_info_cardtype3->id;
        }
        foreach($mall_vipcards_cardtype3 as $k=>$v){
            $id = $v->id;
            if(empty($usageslists_cardtype3[$id]) || $usageslists_cardtype3[$id]==‘’){
                unset($mall_vipcards_cardtype3[$k]);
            }
        }

    $mall_vipcards = array_merge($mall_vipcards,$mall_vipcards_cardtype3);
	...
	if (count($mall_vipcards) > 0)
	{
		...
		foreach($mall_vipcards as $mall_vipcard_info)
		{
            ...
            if($ids[$id]>1){$vipcardlist[$id]['num'] = $ids[$id];}
...
foreach($vipcardlist as $k=>$v){
    for ($i=1; $i < $v['num']; $i++) {
        array_push($vipcardlist,$v);
    }
}
...
$smarty->assign('supplierids',$_SESSION['supplierid']);
