		...
		<div class="mui-off-canvas-wrap mui-draggable" id="offCanvasWrapper">
			<div class="mui-inner-wrap">
				<header class="mui-bar mui-bar-nav" style="padding-right: 15px;">
					 ...
					 <h1 class="mui-title" id="pagetitle">自提发货</h1>
				</header>
                <nav class="mui-bar mui-bar-tab">
                	<a class="mui-tab-item mui-active" href="/vendorsendConfirm.php?record={$orderid}&supplierid={$supplierid}">
                		<span class="mui-tab-label">确认发货</span>
                	</a>
                </nav>
						         ...
						         <ul class="mui-table-view">
			                                <li class="mui-table-view-cell">
													<div class="mui-media-body  mui-pull-left">
														<span class="mui-table-view-label">订单状态：</span><span class="ordersn blue">{$orderinfo.order_status}</span><br>
											...
											{if $orderinfo.delivery neq ''}
                                                ...
                                                {foreach item=invoicenumber_info from=$orderinfo.deliverytime key=k}

                                                    <li class="mui-table-view-cell">
                                                            <div class="mui-media-body">
                                                                <span class="mui-table-view-label">发货时间：</span><span class="ordersn">{$orderinfo.deliverytime.$k}</span><br>
                                                                <span class="mui-table-view-label">物流公司：</span><span class="ordersn">{$orderinfo.deliveryname.$k}</span><br>
                                                                <span class="mui-table-view-label">发货单号：</span><span class="ordersn">{$orderinfo.invoicenumber.$k}</span><br>
                                                            </div>
                                                    </li>



                                                    {if $orderinfo.invoicenumber.$k neq 'JD'}
                                                    <li class="mui-table-view-cell">
                                                            <div class="mui-media-body ">
                                                                <a id="wuliu" href="wuliu.php?type={$orderinfo.deliveryname.$k}&invoicenumber={$orderinfo.invoicenumber.$k}" style="width:100%"><h5 class="show-content" style="padding: 10px;">物流信息【点击查看物流信息】</h5></a>
                                                            </div>
                                                    </li>
                                                    {else}
                                                        {foreach name="invoicenumbers" item=invoicenumber_info  from=$orderinfo.invoicenumbers}
                                                             <li class="mui-table-view-cell">
                                                                <div class="mui-media-body ">
                                                                    <a id="wuliu" href="wuliu.php?type={$orderinfo.deliveryname}&invoicenumber={$invoicenumber_info}" style="width:100%"><h5 class="show-content" style="padding: 10px;">物流信息【点击查看物流信息】</h5></a>
                                                                </div>
                                                        </li>
                                                        {/foreach}
                                                    {/if}
			                                ...
			                                <li class="mui-table-view-cell">
													<div class="mui-media-body  mui-pull-left">
														...
														<span class="mui-table-view-label">余额支付：</span>
														    <span class="totalpricse">￥<span>{$orderinfo.usemoney}</span>
														</span><br>
														<span class="mui-table-view-label">卡券优惠：</span>
														    <span class="totalprice">￥<span>{$orderinfo.discount}</span>
														</span><br>
                                                        {if $amount neq null}
                                                        <span class="mui-table-view-label" style="height: 20px;width: 74px">商城卡支付：</span>
                                                        <span class="totalprice">￥<span>{$amount}</span>
														</span><br>
                                                        {/if}
														<span class="mui-table-view-label">实付金额：</span>
														    <span class="totalprice">￥<span>{$orderinfo.paymentamount}</span>
														</span>
													</div>
			                                 </li>
											{/if}
			                                <li class="mui-table-view-cell">
													<div class="mui-media-body  mui-pull-left">
														<span class="mui-table-view-label">订单总额：</span>
														    <span class="totalprice">￥{$orderinfo.sumorderstotal}
														</span>（邮费{$orderinfo.orders_products.0.postage}元）<br>
									  ...
									  {foreach name="orders_products" item=orders_products_info  from=$orderinfo.orders_products}
		  									<li class="mui-table-view-cell mui-left" style="min-height:104px;">
  												...
  												<div class="mui-pull-right" style="margin-right: 10px">
  													<span>{if $orders_products_info.zhekou neq '' && $orders_products_info.zhekou|floatval gt 0 }{if $orders_products_info.activitymode eq '1'}{else}{/if}<span class="price">¥{$orders_products_info.shop_price}</span> <span style="color:#878787;margin-left:5px;text-decoration:line-through;">¥{$orders_products_info.old_shop_price}</span>{else}<span class="price">¥{$orders_products_info.shop_price}</span>{/if}</span><br>
