					...
					<ul class="mui-table-view">
						<li class="mui-table-view-cell mui-media mysure_list">
							<div class="mui-media-body">
								<p class="mysure_user" style="text-indent: 17px;font-weight: bold;">{$deliveraddress.consignee}<span style="14px;">{$deliveraddress.mobile}</span></p>
								<p><span class="icon icon-address" style="font-size: 17px;">&#xe600;</span>{$deliveraddress.province}{$deliveraddress.city}{$deliveraddress.district}{$deliveraddress.shortaddress}
								</p>
							</div>
						...
						<li class="mui-table-view-cell" style="margin-top: 10px;">
							<div class="mui-media-body  mui-pull-right">
								<span class="mui-table-view-label">总金额： </span><span class="totalprice">￥<span style="font-size: 1em;">{$total_money}</span></span>
							</div>
						</li>
						<div id="paymentgroup">
							{if $total_money ge 50}
							{if $profile_info.money gt 0}
							<li class="mui-table-view-cell price-yue">
								<div class="mui-media-body  mui-pull-right">
									<span class="mui-table-view-label">您的余额： </span><span class="totalprice">￥<span class="yuecanuse" style="font-size: 1em;">{$profile_info.money}</span></span>
								</div>
							</li>
							...
							<li class="mui-table-view-cell">
								<div class="mui-media-body  mui-pull-right">
									...
									<div class="mui-pull-right" style="height:26px;line-height:26px;text-align: right">
										【本单最大可用余额为<span class="totalprice" id="allowmoney">￥{$availablenumber}</span>】
									</div>
								</div>
							</li>
							{/if}
							...
							{/if}
							{/if}
							{if $total_money lt 50}
							{if $profile_info.money gt 0}
							<li class="mui-table-view-cell price-yue"  style="background-color: #ccc;color: #999;">
								<div class="mui-media-body  mui-pull-right">
									<span class="mui-table-view-label">您的余额： </span><span class="totalprice" style="color: #999 !important;">￥<span class="yuecanuse" style="font-size: 1em;">{$profile_info.money}</span></span>
								</div>
							</li>
							{if $frozenstatus eq 'Frozen'}
							...
							<li class="mui-table-view-cell"  style="background-color: #ccc;color: #999;">
								<div class="mui-media-body  mui-pull-right">
									<div class="mui-checkbox mui-left mui-pull-left" style="width:90px;height:26px;line-height:26px;">
										<label>使用余额</label>
										<span disabled {if $frozenstatus eq 'Frozen'}disabled{/if} type="checkbox"></span>
									</div>
									<div class="mui-pull-right" style="height:26px;line-height:26px;text-align: right;">
										【本单余额最大可用为<span class="totalprice" id="allowmoney" style="color: #999 !important;">￥{$availablenumber}</span>】
									</div>
								</div>
							...
							{/if}
							{/if}
							{if $vipcardusagelist|@count gt 0 && $total_money ge $availablenumber}
							<li class="mui-table-view-cell">
								<a href="#vipcards" class="mui-navigate-right vipcards">
									<div class="mui-media-body  mui-pull-left">
										<span class="mui-table-view-label">卡券优惠：</span>
										<span id="vipcard_msg">{$vipcard_usage_info.vipcardname}</span>
									</div>
								</a>
							</li>
							<li class="mui-table-view-cell">
								<div class="mui-media-body  mui-pull-left">
									<span class="mui-table-view-label">优惠金额：</span>
										<span class="totalprice">￥
												<span style="font-size: 1em;" id="discount">{$vipcard_usage_info.amount}</span>
										</span>
								</div>
							</li>
							{else}
							...
							{/if}
							{if $official neq 'true'}
							<li class="mui-table-view-cell price-yue">
								<div class="mui-media-body  mui-pull-right">
									<span class="mui-table-view-label" style="display: inline-block;width:75px;">惠民商城卡： </span>
									<!-- <a href="smk_bindcard.php?record={$orderid}" class="mui-btn mui-btn-danger" style="font-size: 12px;width:70px;height:22px;text-align: center;padding-top:2px;background-color: #f00;">绑定新卡</a> -->
									<span class="totalprice">￥<span style="font-size: 1em;" id="sckmoney">{if $totle_money eq null}0.00{else}{$totle_money}{/if}</span></span>
								</div>
							</li>
							<li class="mui-table-view-cell">
								<div class="mui-media-body  mui-pull-right">
								...
								<input id="totle_money" name="totle_money" value="{$totle_money}" type="hidden">
							...
							<li class="mui-table-view-cell">
								<div class="mui-media-body  mui-pull-left">
									...
									<span class="totalprice">￥
											<span style="font-size: 1em;" id="needpayment">{if $vipcard_usage_info|@count gt 0}{$vipcard_usage_info.total_money}{else}{$total_money}{/if}</span>
									</span>
								</div>
							</li>
							<li class="mui-table-view-cell" id="paymentwaygroup" >
								...
								<div class="yanse_xz mui-pull-left" id="radiogroup">
									...
									{if $official eq 'true' && $official_in eq 'true'}
									...
									{/if}
									{if $official eq 'true' && $shidubill_in eq 'true'}
                                    {*史嘟通宝支付*}
							...
							<li id="wxsmk_cardno_div" class="mui-table-view-cell" style="display:block;" >
								<div class="mui-media-body  mui-pull-left">
									...
									<input id="wxsmk_cardno" name="wxsmk_cardno"
										   value="{$userlast_card}" type="text" required="required"
			...
			<div class="promote-left-part">
				<div class="inner">
					<div class="promote-card-value">
						...
						<i id="myamount">{$vipcardusage_info.amount}</i>
					</div>
					<div class="promote-condition font-size-12"><p id="myorderamount" style="color:#fff;">
						{if $vipcardusage_info.orderamount eq '0'}
							{if $vipcardusage_info.cardtype neq '3'}
							无门槛优惠券
							{/if}
						{else}
						满{$vipcardusage_info.orderamount}元可用
						{/if}
					</p><p id="mytimelimit" style="color:#fff;">{if $vipcardusage_info.timelimit eq '0'}限次使用{else}不限次使用{/if}</p></div>
				</div>
			</div>
			<div class="promote-right-part left font-size-12">
				<div class="inner">
					<h5 style="font-size: 1.2em;text-align: left;">{$vipcardusage_info.vipcardname}</h5>
						<a class="mui-radio"  style="margin:18px 0px 6px 60px;"   href="#" data-id="{$vipcardusage_info.id}" data-name="{$vipcardusage_info.vipcardname}" data-amount="{$vipcardusage_info.amount}" data-orderamount="{if $vipcardusage_info.orderamount eq '0'}无门槛优惠券{else}
						满{$vipcardusage_info.orderamount}元可用{/if}" data-timelimit="{if $vipcardusage_info.timelimit eq '0'}限次{else}不限次{/if}">
							<!--zl优惠券是否有效-->
							<input  class="useticket" style="right:4px;" name="useticket" type="radio" value="0">
						</a>
					<!--~有效时间-->
					<div class="promote-condition font-size-12">{$vipcardusage_info.starttime} 到 {$vipcardusage_info.endtime}</div>
				</div>
		...
		<!-- <li class="promote-item coupon-style-0">
				...
				<div class="promote-right-part left font-size-12">
					<div class="inner">
						...
						<a class="mui-radio"  style="margin:18px 0px 6px 60px;"   href="#" data-id="{$vipcardusage_info.id}" data-name="111" data-amount="222" data-orderamount="无门槛优惠券" data-timelimit="{if $vipcardusage_info.timelimit eq '0'}限次{else}不限次{/if}">
							<input  class="useticket" style="right:4px;" name="useticket" type="radio" value="0">
						</a>
...
<script type="text/javascript">
	...
	var autoriza_sign = "{$autoriza_sign}";
 	{literal}
	mui.init({
