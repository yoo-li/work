		...
		mui.ready(function() {
			...
			mui('.mui-inner-wrap').on('tap','button',function(e){
				...
				if(mychoice.checked) {
					...
					if(card.length==15) {
						mui.ajax('/smk_bindticket.ajax.php', {
							data: {
								card: card, //券号
								word: word  //密码
							},
							...
							success: function (data) {
								...
								if (data.id == 200) {
									mui.alert("恭喜您，兑换成功！", " ", "确定", function (e) {
										mui("#bind")[0].removeAttribute("disabled");
										mui("#bind")[0].innerHTML = "兑换";
										mui.openWindow({
											url: "coupons_view.php?record="+data.record
										})
									});
								} else {
									//登陆失败；
									mui.alert("券号或密码错误，请重新提交！", " ", "确定");
									mui("#bind")[0].removeAttribute("disabled");
									mui("#bind")[0].innerHTML = "兑换";
								}
							},
	...
	<div class="mui-inner-wrap" style="background-color:#fff;">
		...
		<div style="padding:50px 8px 100px 8px;">
			<form class="mui-input-group"  style="font-size: 16px">
				<br>
				请输入兑换券编号:<br>
				<input type="number" id="card" class="mui-input-clear" placeholder="请输入兑换券背面编号"  style="border:1px solid #aaa;background-color:#fff;margin-top: 10px;font-size: 15px;"><br><br>
				请输入兑换券密码:<br><br>
				<input type="text" id="word" class="mui-input-clear" placeholder="请刮兑换券背面图层输入密码"  style="border:1px solid #aaa;background-color:#fff;margin:-8px auto;font-size: 15px;"><br><br><br>
				<div style="text-align: center;">
					...
					<span  style="font-size: 12px;">您已阅读并同意<a href="smk_ticketagreement.php" style="color:#f00;text-decoration: underline">《兑换券使用许可协议》</a></span>
				</div>
				<button id="bind" type="button" class="mui-btn mui-btn-danger"  style="border-radius: 0px;width:100%;margin-top:10px;">兑换</button>
			</form>
			<br>
			<br><br>
		</div>
