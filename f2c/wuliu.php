...
if($type == "京东")
{
	...
	$ordertracks = JD::getordertracks($invoicenumber); 
	
	$logisticroutes = array();
	$pos = 0;
	foreach($ordertracks as $ordertrack_info)
	{
		$msgTime = $ordertrack_info['msgTime'];
		$date = date("Y-m-d",strtotime($msgTime));
		$time = date("H:i:s",strtotime($msgTime));
		if ($pos == 0)
		{
			$logisticroutes[] = array("pos"=>'start','date'=>$date,'time'=>$time,'route'=>$ordertrack_info['content']);
		}
		else if ($pos == count($ordertracks) - 1)
		{
			$logisticroutes[] = array("pos"=>'end','date'=>$date,'time'=>$time,'route'=>$ordertrack_info['content']);		
		} 
		else
		{
			$logisticroutes[] = array("pos"=>$pos,'date'=>$date,'time'=>$time,'route'=>$ordertrack_info['content']);
		}
		$pos ++;
	}
