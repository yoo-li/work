...
if( isset($_SESSION['miao_token']) && $_SESSION['miao_token']!=''){
//    var_dump($_SESSION);die;
    $miao_id = $_SESSION['miao_id'];
    $_REQUEST['token'] = $_SESSION['miao_token'];
    $ArrayList = array();
    $ArrayList[] = $miao_id;
    $_REQUEST["shoppingcart"] = $ArrayList;
    $_REQUEST["qty_item_$miao_id"] = '1';
    $_REQUEST["miao_price"] = $_SESSION['miao_price'];
    $miao_price =  $_SESSION['miao_price'];
	...
	try
	{
		...
		if (isset($_REQUEST['record']) && $_REQUEST['record'] != '')
		{
			...
			if (count($skuids) > 0)
			{
				...
				foreach ($stocks as $skuid => $stock_info)
				{
					if ($stock_info['state'] != "33" && $allowpayment == "true")
					{
						$allowpayment = "false";
						$errormsg = "【".$skuproducts[$skuid]."】的京东库存不足，无法确认订单！";
						break;
					}
				}
			}
		...
		elseif(isset($token) && !empty($token))
		{
			...
			else
			{
				...
				foreach ($shoppingcarts as $shoppingcart_info)
				{
					...
					$skuid                 = $shoppingcart_info->my->skuid;
                   ...
                   if (isset($_REQUEST["miao_price"])  && $_REQUEST["miao_price"]!= ''){
                       $shop_price            = $_REQUEST["miao_price"];
                       }else{
                       $shop_price            = $shoppingcart_info->my->shop_price;
                   }
//var_dump($shop_price);die;
//                    die;
...
if($shoppingcarts){$vendorid = $shoppingcarts[0]->my->vendorid;}
if($orders_products){$vendorid = $orders_products[0]->my->vendorid;}

$vendorid = '45924';
	$vendor_info = XN_Content::load($vendorid,"mall_vendors_".$supplierid);
	$vendorname = $vendor_info->my->vendorname;
	if ($supplierid == '12352'&& $vendorname == 'jdvop')
	{
		//print_r($shoppingcart);
		//echo '<br>';
		//print_r($deliveraddressinfo);
		//header('Content-Type:text/html;charset=utf-8');
		
		
		$address = $deliveraddressinfo['address'];
		$shortaddress =  $deliveraddressinfo['shortaddress'];
		$province =  $deliveraddressinfo['province'];
		$city =  $deliveraddressinfo['city'];
		$district = $deliveraddressinfo['district']; 
...
//					$towns = JD::gettownsbycountyid($districtid);
		...
		//echo '___'.$provinceid.'____'.$cityid.'_____'.$districtid.'___'.$townid.'_______<br>';
		
		$skuids = array();
		 		
		foreach ($shoppingcart as $shoppingcart_info)
		{ 
			$skuids[] = $shoppingcart_info['skuid'];
		}
		
		require_once(dirname(__FILE__) . "/jd.func.php"); 
		
		$arealimit = JD::checkarealimit($skuids,$provinceid,$cityid,$districtid,$townid);
		
		foreach($arealimit as $arealimit_info)
		{ 
			if ($arealimit_info->isAreaRestrict == 1)
			{
				foreach ($shoppingcart as $shoppingcart_info)
				{ 
					if ($arealimit_info->skuId == $shoppingcart_info['skuid'])
					{
						$productname = $shoppingcart_info['productname'];
						$errormsg = '商品['.$productname.']受区域限制,不能购买.';
						...
						$smarty->assign("aa", number_format($goods_money, 2, ".", ""));
						...
						$allowpayment = "false";
						$smarty->assign("allowpayment", $allowpayment);
						$smarty->assign("errormsg", $errormsg);  
					    $smarty->display('smk_'.$action.'.tpl');
					    die();
					}
	...
	if( isset($miao_price) && $miao_price != ''){
	    if($miao_price <= 119){
            $miao_price += $addpostage + $allpostage + $allmergepostage;
        }else{
            $miao_price = $miao_price;
        }
        $smarty->assign("total_money", $miao_price);
    }else{
...
//var_dump(333);die;
	...
	$smarty->assign("salesactivityid", $_GET['salesactivityid']);
//die;


if($supplierid ==  '12352'){
//    $smarty->display('smk_'.$action.'.tpl'); //smk_miao_submitorder.tpl

    if( isset($miao_price) && $miao_price !=''  ){
        $smarty->assign("miao_price", $miao_price);
//        var_dump(11);die;
        $smarty->display('miao_smk_submitorder.tpl');
        die();
    }else{
//        var_dump(22);die;
        $smarty->display('smk_submitorder.tpl');
        die();
    }
