...
function  mall_orders($profileid,$supplierid,$page,$type='trade')
{
	...
	{
		$robOrderproducts= XN_Query::create ( 'YearContent' )
			->tag('mall_orders_products_'.$profileid)
			->filter ( 'type', 'eic', 'mall_orders_products')
			->filter ( 'my.deleted', '=', '0')
			->filter ( 'my.supplierid', '=', $supplierid)
			->filter ( 'my.profileid', '=', $profileid)
			->filter ( 'my.tradestatus', '=', 'notrade')
			->filter ( 'my.robproductid','>','0')
			->end(100)
			->execute ();
		if(count($robOrderproducts)){
			foreach($robOrderproducts as $orderproduct){
				$orderproduct->my->robproductid='';
				$orderproduct->my->robsingledetailid='';
				$orderproduct->my->activitymode='';
				$orderproduct->my->shop_price=$orderproduct->my->old_shop_price;
				$orderproduct->my->total_price=$orderproduct->my->old_shop_price;
				$order_info=XN_Content::load($orderproduct->my->orderid,"mall_orders_".$profileid,7);
				$alltotle_money=$orderproduct->my->old_shop_price+$order_info->my->postage;
				$total_money=$orderproduct->my->old_shop_price;
				$order_info->my->sumorderstotal   = number_format($alltotle_money, 2, ".", "");
				$order_info->my->amountpayable    = number_format($alltotle_money, 2, ".", "");
				$order_info->my->orderstotal   = number_format($total_money, 2, ".", "");
				$order_info->save("mall_orders,mall_orders_".$profileid);
			}
			XN_Content::batchsave($robOrderproducts,'mall_orders_products,mall_orders_products_'.$profileid);
		}

		//待付款，检查订单详情，如果订单为秒杀订单，则订单要回复为原价
		$orders = XN_Query::create ( 'YearContent' )->tag('mall_orders_'.$profileid)
					->filter ( 'type', 'eic', 'mall_orders')
					->filter ( 'my.deleted', '=', '0')
					->filter ( 'my.supplierid', '=', $supplierid)
	...
	if (count($orders) > 0)
	{
		...
		foreach($orders as $order_info)
		{
			...
			$orderlist[$id]['smkaddressid'] = $order_info->my->smkadressid;

			$orderlist[$id]['delivery_status'] = $order_info->my->delivery_status;
			$orderlist[$id]['deliverytime'] = $order_info->my->deliverytime;

            $orderlist[$id]['officialapprovalstatus'] = $order_info->my->officialapprovalstatus;
