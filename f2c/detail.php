...
$salesactivityid = $_GET['salesactivityid'];
...
if(time()>=strtotime($result[0]->my->begindate) && time()<strtotime($result[0]->my->enddate)){
    $statue=1;//正在进行中
}
elseif(time()<strtotime($result[0]->my->begindate)){
    $statue=0;//还没开始
}
else{
    $statue=-1;//已结束
}
$smarty->assign("statue", $statue);
...
foreach ( $resuult as  $key=>$value){
    $list[] = $value->my->salesactivityid;//获取参与活动ID
}
//活动及其链接
$res_mall_salesactivitys = XN_Query::create('Content')->tag('mall_salesactivitys')
    ...
    ->execute();
 $res_list = array();
foreach ( $res_mall_salesactivitys as  $key=>$value){
    $res_list[$key]['activityname'] = $value->my->activityname;//获取参与活动名称
    $res_list[$key]['url'] = "detail.php?from=salesactivity&productid="."$productid"."&salesactivityid=".$value->id;
...
$smarty->assign("activity_info", $res_list);
$sign =  count($res_mall_salesactivitys);
$smarty->assign("sign", $sign);    //是否存在的活动标识

 if($supplierid !=  '8596'){
    if($miao_statue == 1){
//        传递秒杀的商品价格 数量
        if(isset($_GET['salesactivityid']) && $_GET['salesactivityid']!= ''){
            $salesactivityid = $_GET['salesactivityid'];
            $result = XN_Query::create ( 'Content' )->tag ( 'Mall_RobSingles_details' ) // 514968水蜜桃
                ->filter ( 'type', 'eic', 'Mall_RobSingles_details' )
                ->filter ( 'my.salesactivityid', '=', $salesactivityid )
                ->filter ( 'my.productid', '=', $_GET['productid'] )
                ->execute();
            $miao_price = $result[0]->my->robprice;
            $miao_number = $result[0]->my->activitynumber;
            $begindate = $result[0]->my->begindate;
            $enddate = $result[0]->my->enddate;
            $record = $result[0]->id;
            ...
            $robproducts=XN_Query::create("Content")
                ...
                ->filter("my.deleted",'=','0')
                ->execute();
            if(count($robproducts)){
                $robproduct_info=$robproducts[0];
                if($robproduct_info->my->robpaymentstatus=='未支付')
                    $smarty->assign("rob_status",'1');
                else
                    $smarty->assign("rob_status",'2');
            }
            else{
                $smarty->assign("rob_status",'0');
            }
            $smarty->assign("starttime",date("Y-m-d H:i:s",strtotime($begindate)));     //date("Y-m-d H:i:s",strtotime($val->my->begindate))
            $smarty->assign("endtime",date("Y-m-d H:i:s",strtotime($enddate)));     //date("Y-m-d H:i:s",strtotime($val->my->begindate))
            $smarty->assign("miao_price",$miao_price);
            $smarty->assign("miao_number",intval($miao_number));
        ...
        $smarty->assign("miao_id", $miao_id);
        $smarty->assign("salesactivityid", $salesactivityid);
        $_SESSION['miao_token'] = $guid;
        $_SESSION['miao_price'] = $miao_price;

        $ArrayList = array();
        $ArrayList[] = $miao_id;
        $_SESSION['shoppingcart'] = $ArrayList;
        $_SESSION['miao_id'] = $miao_id;
