...
class XN_REST {
   ...
   protected function doRequest($method, $url, &$body = null,$contentType=null, $additionalHeaders = null) {
       static $eventRequestId = 0;
       $requestHeaders = array();

       // Prevent curl from automatically adding Expect: 100-continue, as this can cause
       // Apache Abdera to return two HTTP status codes, e.g., 100 and 201.
       // See http://curl.haxx.se/mail/lib-2004-08/0241.html  [Jon Aquino 2007-11-20]
       ...
       /* Don't add a request token if we're using the __other__ slot which makes
        * requests outside the playground or the GLOBAL_API_HOST_PORT slot which
        * hits the api.* endpoint */
       /* Only add a request token if we're using the local slot (NING-5606) */
       if (($urlInfo['slot'] !== '__other__') &&
           ($urlInfo['slot'] !== '__global__')) {
           $requestHeaders['X-Ning-RequestToken'] = XN_REST::$SECURITY_TOKEN;
       }

       /* If the request came in with tracing enabled, ensure that the tracing
        * parameter is sent back with API calls */
       /* NING-8754: this is allowed for use on a per-request basis */
       if ($traceEnabled = isset($_GET['xn_trace'])) {
           if ($urlInfo['slot'] != '__other__') {
               $urlInfo['url'] .= ((strpos($urlInfo['url'], '?') === false) ? '?' : '&').'xn_trace=true';
           }
       }
       
       /* Tracing may be enabled through the X-XN-Trace header (NING-8195) */
       /* NING-8754: this is disabled until upstream can handle long response headers */
       /*        if (isset($_SERVER['HTTP_X_XN_TRACE']) && ($_SERVER['HTTP_X_XN_TRACE'] == 'true')) {
            $traceEnabled = true;
            $requestHeaders['X-XN-Trace'] = 'true';
            }*/

       /* Add the X-XN-Trace-Token header to any requests made to internal
        * servers (i.e. everything but the __other__ slot (NING-6886) */
       if (($urlInfo['slot'] != '__other__') && isset(self::$TRACE)) {
           $requestHeaders['X-XN-Trace-Token'] = self::$TRACE;
       }
	   ...
	   * responses with no body (NING-7074) */
        ...
        /* Handle any before-request activities -- hooks, timing */
        XN_Event::fire('xn/rest/request/before', array($eventRequestId, $this->curl[$slot], $requestHeaders, $body));
        if ($traceEnabled) {
            $timeBeforeRequest = microtime(true);
        }
        if (XN_Debug::isDebugEnabled()) {
            $debug['elapsed'] = microtime(true);
        }

        /* Make the mighty request */
        $res = @curl_exec($this->curl[$slot]);

        /* Handle after-request activities -- hooks, timing */
        if (XN_Debug::isDebugEnabled()) {
            $debug['request-headers'] = $requestHeaders;
            $debug['response-headers'] = $this->headers;
            $debug['response-body'] = $this->body;
            $debug['response-code'] = $this->responseCode;
            $debug['elapsed'] = microtime(true) - $debug['elapsed'];
            XN_Debug::debug($debug);
        }
        XN_Event::fire('xn/rest/request/after', array($eventRequestId, $this->curl[$slot], $this->responseCode, $this->headers, $this->body));
        ...
        /* Set up the XN-ResponseFrom header if tracing is enabled */
        if ($traceEnabled) {
            $this->setTraceResponseHeader($url, $timeBeforeRequest);
        }
        ...
        if ($res === false) {
	        ...
	        header('Content-Type:text/html;charset=utf-8');
			if ($_SERVER["SERVER_ADDR"] == "124.232.138.140")
			{
				$this->errorprint('警告',$e->getMessage().'<br><br><div style="text-align: left;padding-left: 30px;"> 长沙千里及网络技术有限公司<br>Copyright © 2010-2014 ttwz168.com <br>紧急联系手机：15367890109(关云长),15974160308(孔明)<br></div>');	
 	        	
			}
			else
			{
				$this->errorprint('警告',$e->getMessage().'<br><br><div style="text-align: left;padding-left: 30px;"> 湖南赛明威科技有限公司<br>Copyright © 2010-2014 saasw.com <br>紧急联系手机：15974160308(王真明)<br></div>');	
 	    	}
			die();			   
            throw $e;
        }
