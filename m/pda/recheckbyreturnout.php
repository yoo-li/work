...
//	XN_REST::$LOCAL_API_HOST_PORT = $_SERVER['HTTP_X_NING_LOCAL_API_HOST_PORT'];
	...
	$recheckordersid = 0;
	$loadContent = XN_Content::load($record,"ma_inventoryreturnout");
	...
	$ma_returnordersout=$loadContent->my->ma_returnordersout;
	...
	foreach($pickorders as $pickorder_info){
		...
		$query=XN_Query::create("Content")
			->tag("ma_recheckordersout")
			->filter("type","eic","ma_recheckordersout")
			->filter("my.ma_returnordersout","=",$ma_returnordersout)
			->filter("my.deleted","=","0")
			->execute();
		if(count($query)){
			...
			$recheck_order->my->saleinfos=$loadContent->my->saleinfos;
			$recheck_order->my->purchaseinfos=$loadContent->my->purchaseinfos;
			$recheck_order->save("ma_recheckordersout");
			$recheckordersid=$recheck_order->id;
		}
		else
		{
			$recheck_order=XN_Content::create("ma_recheckordersout","",false);
			$recheck_order->my->supplierid=$submit_id;
			$recheck_order->my->send_supplierid=$loadContent->my->send_supplierid;
			$recheck_order->my->ma_returnordersout=$loadContent->my->ma_returnordersout;
			$recheck_order->my->ma_returnordersin=$loadContent->my->ma_returnordersin;
			$recheck_order->my->number=$pickorder_info->my->submitnumber;
	...
	$certificateout->my->supplierid=$submit_id;
	...
	$certificateout->my->ma_returnordersout=$loadContent->my->ma_returnordersout;
	$certificateout->my->ma_returnordersin=$loadContent->my->ma_returnordersin;
	$certificateout->my->ma_inventoryreturnout=$record;
	$certificateout->my->certificate_type="returnout";
	...
	$returnout_details=XN_Query::create("Content")
		->tag("ma_returnoutdetails")
		->filter("type","eic","ma_returnoutdetails")
		->filter("my.record","=",$record)
		->filter("my.deleted","=","0")
		->execute();
	$factorys_names=array();
	foreach($returnout_details as $returnout_detail){
		$returnout_detail->my->submitnumber+=$product_submitnumbers[$returnout_detail->my->ma_products];
		$factorys_names[$returnout_detail->my->factorys]=$returnout_detail->my->factorys_name;
		$returnout_detail->save("ma_returnoutdetails");
	}
	...
	$loadContent->save("ma_inventoryreturnout");
	...
	foreach($pickdetails as $detail_info){
		...
		foreach($batch_infos as $index=>$number_arr){
			...
			$billwaterContent->my->ma_returnordersout=$ma_returnordersout;
			$billwaterContent->my->ma_returnordersin=$ma_returnordersin;
	...
	foreach(array_chunk($billwater_Datas,50) as $child_billwater_Datas){
		$child_billwater_Datas->save("ma_inventorybillwaters");
	}
