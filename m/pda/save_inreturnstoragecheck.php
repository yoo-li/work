...
//    $ma_incheckstorage = $instorage_info->my->ma_incheckstorage;
            ...
            if ($myproducts_batch_no === $binfo["products_batch_no"])
            {
                ...
                if ($takepartin === "1")
                {
                    if(isset($binfo["add_checknumber"])){
                        $number+=$binfo["add_checknumber"];
                    }
                    $oldbatch_infos[$key]["add_checknumber"]  = $number;
                    $oldbatch_infos[$key]["add_refusenumber"] = $oldbatch_infos[$key]["ing_instoragenumber"]-$number;
                ...
                else
                {
                    ...
                    {
                        $number       = $oldbatch_infos[$key]["instoragenumber"];
                        $refusenumber = "0";
                    }else{
                        $number+=$oldbatch_infos[$key]["checknumber"];
                        $refusenumber=$oldbatch_infos[$key]["instoragenumber"]-$number;
                    }
    ...
    $loadcontent              = XN_Content::load($record, "ma_inreturnstoragecheck");
    $loadcontent->my->execute = $profileid;
    $loadcontent->save("ma_inreturnstoragecheck");
    echo '200';
