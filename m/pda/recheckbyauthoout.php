...
//	XN_REST::$LOCAL_API_HOST_PORT = $_SERVER['HTTP_X_NING_LOCAL_API_HOST_PORT'];
	...
	$recheckordersid = 0;
	$loadContent = XN_Content::load($record,ma_inventoryauthoout);
	$ma_authoallocatecontract=$loadContent->my->ma_authoallocatecontract;
	...
	foreach($pickorders as $pickorder_info){
		...
		if(count($query)){
			...
			$recheck_order->save("ma_recheckordersautho");
			$recheckordersid=$recheck_order->id;
		}
		else
		{
			$recheck_order=XN_Content::create("ma_recheckordersautho","",false);
			$recheck_order->my->send_supplierid=$loadContent->my->send_supplierid;
			$recheck_order->my->check_supplierid=$loadContent->my->check_supplierid;
			$recheck_order->my->supplierid=$loadContent->my->supplierid;
			$recheck_order->my->submit_type=$loadContent->my->submit_type;
			$recheck_order->my->submit_id=$loadContent->my->submit_id;
			$recheck_order->my->receipt_type=$loadContent->my->receipt_id;
			$recheck_order->my->receipt_id=$loadContent->my->receipt_id;
			$recheck_order->my->ma_authoallocatecontract=$ma_authoallocatecontract;
			$recheck_order->my->ma_inventoryauthoout=$record;
			$recheck_order->my->delivery_status="1";
			$recheck_order->my->number=$pickorder_info->my->submitnumber;
	...
	$certificateout->my->supplierid=$loadContent->my->supplierid;
	...
	$certificateout->my->ma_authoallocatecontract=$ma_authoallocatecontract;
	$certificateout->my->ma_inventoryauthoout=$record;
	$certificateout->my->certificate_type="backout";
	...
	$authoout_details=XN_Query::create("Content")
		->tag("ma_authooutdetails")
		->filter("type","eic","ma_authooutdetails")
		->filter("my.record","=",$record)
		->filter("my.deleted","=","0")
		->end(-1)
		->execute();
	$factorys_names=array();
	foreach($authoout_details as $authoout_detail){
		$authoout_detail->my->submitnumber+=$product_submitnumbers[$authoout_detail->my->ma_products];
		$factorys_names[$authoout_detail->my->factorys]=$authoout_detail->my->factorys_name;
		$authoout_detail->save("ma_authooutdetails");
	}
	...
	foreach($pickdetails as $detail_info){
		...
		foreach($batch_infos as $index=>$number_arr){
			...
			$billwaterContent->my->supplierid=$submit_id;
			$billwaterContent->my->ma_inventoryauthoout=$record;
