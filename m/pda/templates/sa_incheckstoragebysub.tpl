            ...
            {foreach name=blocks key=header item=data from=$incheckstorage}
                <div class="mui-collapse-content products" style="margin: 3px 3px;">
                    ...
                    {foreach  key=key item=info from=$data.batch_info}
                        {if $info.ing_instoragenumber neq '0'}
                            {if $info.submitnumber-$info.add_instoragenumber-$info.ing_instoragenumber>0}
                                ...
                                <div class="mui-input-row" style="height: 60px">
                                    ...
                                    {if $data.suppliername eq '医流通'}
                                        {if $data.factorys_name eq '常州鼎健医疗器械有限公司'}
                                            <input id="productdate_{$data.itemcode}_{$info.products_batch_no}" value="{$info.productdate}"
                                                   type="text"
                                                   readonly=""/>
                                        {else}
                                            <input id="productdate_{$data.barcode}_{$info.products_batch_no}" value="{$info.productdate}"
                                                   type="text"
                                                   readonly=""/>
                                        {/if}
                                    {else}
                            ...
                            {/if}
                        {else}
                            {if $info.submitnumber-$info.add_instoragenumber-$info.ing_instoragenumber-$info.instoragenumber>0}
                                <div class="mui-input-row" style="height: 60px">
                                    <label style="font-size: 15px;">批号：</label>
                                    {if $data.suppliername eq '医流通'}
                                        {if $data.factorys_name eq '常州鼎健医疗器械有限公司'}
...
//        }
        if (document.getElementById("batch_no_" + itemcodes + "_" + batchcodes)) {
            ...
            if (parseInt(document.getElementById("productnum_" + itemcodes + "_" + batchcodes).value) > addbararr.get(result)) {
                ...
                alert(itemcodes+"产品的"+batchcodes+"批号数量大于未收货数!");
                return;
            }
...
//            var itemcodes = str1.substring(3, 19);//条码
//            var batchcodes = str1.substring(29);//批号
//        } else if (str1.substring(0, 3) === 'ATH') {
//            var itemcodes = str1.substring(3, 16);
//            var batchcodes = str1.substring(26);
//        } else if (str1.substring(0, 3) === 'BCL') {
//            var itemcodes = str1.substring(6, 19);
//            var batchcodes = str1.substring(37);
//        } else {
//            var itemcodes = str1.substring(3, 15);//条码
...
//            "sterilizevalidate": sterilizevalidatearr.get(thisresult)
        ...
        mui.ajax({
            ...
            traditional: true,
            dataType: "json",
            url: "save_incheckstoragebysub.php",
