...
//            }
            if(document.getElementById('number_' + codes)){
                var num = parseInt(document.getElementById('number_' + codes).value);
            }else{
                alert("找不到编码为"+codes+"的未出货产品");
                die();
            }
            var addnum = 0;
            for(var j=0;j<i;j++){
...
//        }
    ...
    function getdata(obj) {
            if((js_arr[obj.value].itemcode==obj.value)|| js_arr[obj.value].barcode==obj.value){
                document.getElementById("barcode").value = js_arr[obj.value].barcode;
                document.getElementById("itemcode").value = js_arr[obj.value].itemcode;
                document.getElementById("warehouseinfo").value = js_arr[obj.value].warehouseinfo;
                document.getElementById("number").value = js_arr[obj.value].number;
                document.getElementById("factorys_name").value = js_arr[obj.value].factorys_name;
                if(js_arr[obj.value].suppliername=='医流通'){
    ...
    function savedata(str1,str2){
        ...
        mui.ajax({
            ...
            traditional: true,
            dataType: "json",
            url: "save_borrowincheckstorage.php",
...
//                alert(data);
    ...
    function getproductdate(obj1,obj2,obj3) {
        if (typeof(js_arr[obj2]) != "undefined") {
            var item = js_arr[obj2].batch_info;
            for (var key in item) {
