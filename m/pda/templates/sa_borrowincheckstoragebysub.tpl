                ...
                <div class="mui-collapse-content products" style="margin: 3px 3px;">
                    ...
                    <div class="mui-input-row">
                        ...
                        <div class="mui-numbox" data-numbox-min='0' data-numbox-max='9999999999' style="width:170px">
                            ...
                            {if $data.suppliername eq '医流通'}
                                <input id="productnum_{$data.barcode}_{$info.products_batch_no}" class="mui-input-numbox" type="number" value="{$info.submitnumber}"  onchange="changes(this);"/>
                            {else}
                                <input id="productnum_{$data.itemcode}_{$info.products_batch_no}" class="mui-input-numbox" type="number" value="{$info.submitnumber}"  onchange="changes(this);"/>
                            {/if}
                            <button class="mui-btn mui-btn-numbox-plus" type="button">+</button>
                        </div>
                    </div>
                        {if $data.suppliername eq '医流通'}
                            <input id="hidden_productnum_{$data.barcode}_{$info.products_batch_no}" class="mui-input-numbox" type="hidden" value="{$info.submitnumber}"/>
                        {else}
                            <input id="hidden_productnum_{$data.itemcode}_{$info.products_batch_no}" class="mui-input-numbox" type="hidden" value="{$info.submitnumber}"/>
                        {/if}
                        {/foreach}
                </div>
            {/foreach}
...
<script type="text/javascript" charset="utf-8">
    ...
    mui('body').on('tap', 'button#finishsann', function (e) {
        ...
        mui.ajax({
            ...
            traditional :true,
            dataType:"json",
            url: "save_borrowincheckstoragebysub.php",
    ...
    function getdata(obj) {
        ...
        mui.ajax({
            ...
            dataType: "json",
            url: "find.php",
            data: {"record": record, "barcode": obj.value, "mode":"borrowincheckstorage"},
            success: function (data) {
