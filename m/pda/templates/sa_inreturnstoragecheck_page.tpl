            ...
            {foreach name=blocks key=header item=data from=$instoragecheck}
                <div class="mui-collapse-content products" style="margin: 3px 3px;">
                    ...
                    {foreach  key=key item=info from=$data.batch_info}
                        {if ($info.instoragenumber-$info.checknumber) neq '0'}
                                ...
                                <div class="mui-numbox" data-numbox-min='0' data-numbox-max='9999999999' style="width:170px">
                                    ...
                                    {if $takepartin eq '1'}
                                        {if $data.suppliername eq '医流通'}
                                            <input id="productnum_{$data.barcode}_{$info.products_batch_no}" class="mui-input-numbox"
                                                   type="number"
                                                   value="{$info.ing_instoragenumber-$info.add_checknumber}"/>
                                        {else}
                                    ...
                                    {else}
                                        {if $data.suppliername eq '医流通'}
                                            <input id="productnum_{$data.barcode}_{$info.products_batch_no}" class="mui-input-numbox"
                                                   type="number"
                                                   value="{$info.instoragenumber-$info.checknumber}"/>
                                        {else}
                                            <input id="productnum_{$data.itemcode}_{$info.products_batch_no}" class="mui-input-numbox"
                                                   type="number"
                                                   value="{$info.instoragenumber-$info.checknumber}"/>
...
<script type="text/javascript" charset="utf-8">
    ...
    mui('body').on('tap', 'button#finishsann', function (e) {
        ...
        mui.ajax({
            ...
            data: {"record":ma_checkordersid,"takepartin":takepartin,"mode":"inreturnstoragecheck"},
    ...
    function savedata(str1, str2) {
        ...
        var tempnumber = parseInt(document.getElementById("hidden_productnum_" + itemcodes + "_" + batchcodes).value) - parseInt(str2);
        ...
        mui.ajax({
            ...
            traditional: true,
            dataType: "json",
            url: "save_inreturnstoragecheck.php",
            data: {
                "mybarcodes": fields,
                "record": record,
