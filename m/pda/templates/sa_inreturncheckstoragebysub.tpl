                ...
                <div class="mui-collapse-content products" style="margin: 3px 3px;">
                    ...
                    {foreach  key=key item=info from=$data.batch_info}
                        ...
                        <div class="mui-input-row" style="height: 60px">
                            ...
                            {if $data.suppliername eq '医流通'}
                                <input id="productdate_{$data.barcode}_{$info.products_batch_no}" value="{$info.productdate}" type="text"
                                       readonly=""/>
                            {else}
                                <input id="productdate_{$data.itemcode}_{$info.products_batch_no}" value="{$info.productdate}" type="text"
                                       readonly=""/>
                            {/if}
                        </div>
                        <div class="mui-input-row" style="height: 60px">
                            ...
                            {if $data.suppliername eq '医流通'}
                                <input id="sterilizecode_{$data.barcode}_{$info.products_batch_no}" value="{$info.sterilizecode}"
                                       type="text" readonly=""/>
                            {else}
                                <input id="sterilizecode_{$data.itemcode}_{$info.products_batch_no}" value="{$info.sterilizecode}"
                                       type="text" readonly=""/>
                            {/if}
                        </div>
                        <div class="mui-input-row" style="height: 60px">
                            ...
                            {if $data.suppliername eq '医流通'}
                                <input id="sterilizevalidate_{$data.barcode}_{$info.products_batch_no}" value="{$info.sterilizevalidate}"
                                       type="text" readonly=""/>
                            {else}
                                <input id="sterilizevalidate_{$data.itemcode}_{$info.products_batch_no}" value="{$info.sterilizevalidate}"
                                       type="text" readonly=""/>
                            {/if}
                        </div>
                        <div class="mui-input-row">
                            ...
                            <div class="mui-numbox" data-numbox-min='0' data-numbox-max='9999999999' style="width:170px">
                                ...
                                {if $data.ing_instoragenumber neq '0'}
                                    {if $data.suppliername eq '医流通'}
                                        <input id="productnum_{$data.barcode}_{$info.products_batch_no}" class="mui-input-numbox"
                                               type="number"
                                               value="{$info.submitnumber-$info.add_instoragenumber-$info.ing_instoragenumber}"/>
                                    {else}
                                ...
                                {else}
                                    {if $data.suppliername eq '医流通'}
                                        <input id="productnum_{$data.barcode}_{$info.products_batch_no}" class="mui-input-numbox"
                                               type="number"
                                               value="{$info.submitnumber-$info.add_instoragenumber-$info.ing_instoragenumber-$info.instoragenumber}"/>
                                    {else}
                                        <input id="productnum_{$data.itemcode}_{$info.products_batch_no}" class="mui-input-numbox"
                                               type="number"
                                               value="{$info.submitnumber-$info.add_instoragenumber-$info.ing_instoragenumber-$info.instoragenumber}"/>
...
<script type="text/javascript" charset="utf-8">
    ...
    function JsonResult(result) {
        ...
        if (document.getElementById("batch_no_" + itemcodes + "_" + batchcodes)) {
            if (!contains(mybarcodes, result)) {
                ...
                myarr.put("productnum_" + itemcodes + "_" + batchcodes, result);
            }
            barnum += 1;
            if (parseInt(document.getElementById("productnum_" + itemcodes + "_" + batchcodes).value) > 1) {
                ...
                document.getElementById("productdate_" + itemcodes + "_" + batchcodes).parentNode.remove();
                document.getElementById("sterilizecode_" + itemcodes + "_" + batchcodes).parentNode.remove();
                document.getElementById("sterilizevalidate_" + itemcodes + "_" + batchcodes).parentNode.remove();
        ...
        if (obj.id.indexOf('productnum') == 0) {
            ...
            if (getnums(obj)) {
                ...
                if (checkRate(obj.id)) {
                    var beforenum = parseInt(bararr.get(strid));
                    barnum = barnum - parseInt(obj.value) + beforenum;
                    savedata("WZD" + strid, parseInt(beforenum) - parseInt(obj.value));
                    bararr.put(strid, obj.value);
                }
            } else {
    ...
    function savedata(str1, str2) {
        ...
        if (str1.substring(0, 3) === 'WZD') {
            ...
            var itemcodes = strs[1];//条码
            var batchcodes = strs[2];//批号
        } else if (thisresult.substring(0, 3) === 'YLT') {
            var itemcodes = thisresult.substring(3, 19);//条码
            var batchcodes = thisresult.substring(29);//批号
        } else if (thisresult.substring(0, 3) === 'ATH') {
            var itemcodes = thisresult.substring(3, 16);
            var batchcodes = thisresult.substring(26);
        } else {
            var itemcodes = thisresult.substring(3, 15);//条码
            var batchcodes = thisresult.substring(26);//批号
        }
//        var btnnum = parseInt(bararr.get("_" + itemcodes + "_" + batchcodes));
//        if (btnnum <= 0) {
//            btnnum = 0;
//        }
//        var mynumber = parseInt(document.getElementById("hidden_productnum_" + itemcodes + "_" + batchcodes).value) - parseInt(btnnum);//已复核数量
//        var mysendnumber = parseInt(document.getElementById("hidden_productnum_" + itemcodes + "_" + batchcodes).value);//要复核的数量
        postarr[batchcodes] = {
            "barcode": str1,
            "number": str2,
//            "productdate": productdatearr.get(thisresult),
//            "sterilizecode": sterilizecodearr.get(thisresult),
//            "sterilizevalidate": sterilizevalidatearr.get(thisresult)
        };
        ...
        mui.ajax({
            ...
            traditional: true,
            dataType: "json",
            url: "save_inreturncheckstoragebysub.php",
