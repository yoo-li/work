...
//XN_REST::$LOCAL_API_HOST_PORT = $_SERVER['HTTP_X_NING_LOCAL_API_HOST_PORT'];
    ...
    $detailsinfo           = [];
    $totalnum              = 0;
    $inventorysaleout_info = XN_Content::load($record, "ma_inventoryborrowout");
    $receipt_id             = $inventorysaleout_info->my->receipt_id;
    ...
    {
        $pickorders3 = XN_Query::create("Content")
            ->tag("ma_pickorders")
            ->filter("type", "eic", "ma_pickorders")
            ->filter("my.ma_inventoryborrowout", "=", $record)
            ->filter(XN_Filter::all(XN_Filter("my.delivery_status", "=", "1"), XN_Filter::any(XN_Filter("my.recheck_status", "=", "1"), XN_Filter("my.recheck_status", "=", "2"))))
            ->filter("my.deleted", "=", "0")
            ->execute();
        if (count($pickorders3))
        {
            echo '该订单还没完成复核';
            die();
        }
        ...
        {
            $saleout_info                              = XN_Content::load($inventorysaleout_info->my->ma_borrowordersout, "ma_borrowordersout");
            ...
            $pickorder_info->my->ma_borrowordersin     = $saleout_info->my->ma_borrowordersin;
            $pickorder_info->my->ma_borrowordersout    = $saleout_info->id;
            $pickorder_info->my->ma_inventoryborrowout = $record;
            ...
            $saledetails      = XN_Query::create("Content")
                ...
                ->filter("my.record", "=", $saleout_info->id)
                ->filter("my.deleted", "=", "0")
                ->end(-1)
                ->execute();
    ...
    //根据仓位排序
    $flag = [];
    foreach ($detailsinfo as $arr2)
    {
        $flag[] = $arr2["warehouseinfo"];
    }
    array_multisort($flag, SORT_ASC, $detailsinfo);
    ...
    $smarty->assign("inventoryborrowout", $detailsinfo);
