...
//XN_REST::$LOCAL_API_HOST_PORT = $_SERVER['HTTP_X_NING_LOCAL_API_HOST_PORT'];
        ...
        foreach ($pickorders2 as $pickorder_info)
        {
            ...
            foreach (array_chunk($arr, 50) as $chunk)
            {
                $query = XN_Query::create("Content")
                    ...
                    ->filter("my.author_supplierid", "=", $submit_id)
                    ->filter("my.deleted", "=", "0")
                    ->end(-1)
                    ->execute();
    ...
    //根据仓位排序
    $flag=array();
    foreach($saleoutinfo as $arr2){
        $flag[]=$arr2["warehouseinfo"];
    }
    array_multisort($flag, SORT_ASC, $saleoutinfo);
    ...
    $smarty->assign("inventorysaleoutid", $inventorysaleoutid);
    $smarty->display("sa_inventorysaleout".'.tpl');
?>