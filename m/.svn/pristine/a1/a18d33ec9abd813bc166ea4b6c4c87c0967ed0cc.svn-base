...
//    XN_REST::$LOCAL_API_HOST_PORT               = $_SERVER['HTTP_X_NING_LOCAL_API_HOST_PORT'];
    ...
    $purchaseorders     = XN_Query::create("Content")
        ->tag("ma_inreturncheckstorage")
        ->filter("type", "eic", "ma_inreturncheckstorage")
        ->filter("my.instoragestatus", "in",  ["1", "2"])
//        ->filter("my.receipt_id", "=", $supplierid)
    ...
    {
        $purchaseorders_info  = XN_Content::load($info->my->ma_returnordersin, "ma_returnordersin");
        $ma_purchaseorders_no = $purchaseorders_info->my->ma_returnordersin_no;
