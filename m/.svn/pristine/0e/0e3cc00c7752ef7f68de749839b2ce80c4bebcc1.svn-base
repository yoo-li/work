...
class XN_WX {   
	...
	{ 
	 ...
	 public function initprofile($wxopenid,$update=false)
	 {  
		global $firsttime;
	 							...
	 							if (count($wxopenids) == 0 )	
	 							{
	 								...
	 								if (count($profiles)  ==  0 )	
	 								{
									    ...
									    else 					    
									    {
	 									...
	 									$newcontent->my->profileid = $profile->screenName;
	 ...
	 public function subscribe($appid,$profileid)
	 {
	 ...
	 public function unsubscribe($appid,$profileid)
	 {
		$subscribes = XN_Query::create ( 'Content' ) 
						->filter ( 'type', 'eic', 'subscribes')
						->filter ( 'my.deleted', '=', '0') 
	 ...
	 public function check_profile($wxopenid)
	 {  
 	 	$wxopenids = XN_Query::create ( 'MainContent' )->tag("profile_".$wxopenid)
 	 						->filter ( 'type', 'eic', 'wxopenids')  
 	 						->filter ( 'my.wxopenid', '=', $wxopenid)
	...
	{
	    $url = sprintf('http://api.map.baidu.com/geocoder?output=json&location=%s,%%20%s&key=37492c0ee6f924cb5e934fa08c6b1676',$latitude,$longitude); 
