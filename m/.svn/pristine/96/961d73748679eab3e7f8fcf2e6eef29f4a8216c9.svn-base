		...
		<header class="mui-bar mui-bar-nav" style="padding-right: 15px;">
            ...
            <div class="mui-title mui-content tit-sortbar" id="sortbar"> 
 				<div id="segmentedControl" class="mui-segmented-control mui-segmented-control-inverted mui-segmented-control-negative">
 					<a class="mui-control-item {if $type eq 'unread'}mui-active{/if} iconfont icon-weidu button-color" href="index.php?type=unread">&nbsp;未读公告</a> 
					
					<a class="mui-control-item {if $type eq 'read'}mui-active{/if} iconfont icon-yidu button-color" href="index.php?type=read">&nbsp;全部已读公告</a>
 				</div> 
            </div>
		</header> 
...
<script type="text/javascript">
	...
	mui.ready(function ()
			  {
				  ...
				  mui('.mui-table-view').on('tap', 'a.icon-du', function (e)
				  { 
	...
	function order_html(v)
	{
		...
		sb.append('						<span class="mui-table-view-label">标题：</span>' + v.announcementstitle);
		sb.append('					</div>');  
		sb.append('					<div class="mui-media-body mui-pull-right" style="color:red"> ');
		sb.append('						 '+ v.announcementstype);
		sb.append('					</div> ');
		sb.append('                </li>');
		sb.append('				<li class="mui-table-view-cell">');  
		sb.append('							<div class="mui-media-body">');
		sb.append('								<p class="mui-ellipsis" style="color:#333">' + v.description + '</p>'); 
		sb.append('							</div> '); 
		sb.append('				 </li> ');  
		...
		if (type == "unread")
		{
			...
			sb.append('						 <a class="mui-icon iconfont icon-du button-color" href="index.php?type=unread&markid='+v.announcementid+'" style="padding-right:15px; color:#cc3300;">&nbsp;标记成已读</a>');
			sb.append('					</div> '); 
			sb.append('                </li>');
		}
		sb.append(' 	 	</ul>');
	...
	{
		var sb = new StringBuilder();
		sb.append('<h6 class="mui-content-padded" style="height:0px;margin:0px">&nbsp;</h6>'); 
		sb.append('<div class="mui-card" style="margin: 3px 3px;"> ');
		sb.append('				   		  <p class="tishi"><span class="mui-icon iconfont icon-tishi"></span></p>');
		var type = Zepto('#type').val();
		if (type == "read")
		{
			sb.append('					      <p class="msgbody">您还没有公告已读记录！<br>');
			sb.append('							  <br> ');
		}
		else
		{
			sb.append('					      <p class="msgbody">目前没有新的公告发布！<br>');
			sb.append('							  <br> ');
		} 
		sb.append('						  </p>  ');
		sb.append(' </div>');
	...
	function add_more()
	{
		...
		mui.ajax({
					 type: 'POST',
					 url: "index.php",
					 data: 'type=ajax&t='+type+'&page=' + page,
