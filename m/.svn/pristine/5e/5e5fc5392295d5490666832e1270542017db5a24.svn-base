...
{ 
	if(isset($_REQUEST['page']) && $_REQUEST['page'] != '')
	{
	   ...
	   {   
			...
			$approvallist[$pos]['decider'] = getGivenName($approval_info->my->decider);                    // get username by profieid
			$approvallist[$pos]['authorizedperson'] = getGivenName($approval_info->my->authorizedperson);  // get username by profieid
			$opinion = $approval_info->my->opinion;
			$approvallist[$pos]['opinion'] = join(",",getGivenNameArrByids((array)$opinion));  // // get username by profieid
   
//			$approvallist[$pos]['opinion'] = $opinion;
