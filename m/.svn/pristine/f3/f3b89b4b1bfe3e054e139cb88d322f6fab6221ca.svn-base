...
//	XN_REST::$LOCAL_API_HOST_PORT = $_SERVER['HTTP_X_NING_LOCAL_API_HOST_PORT'];
	...
	$recheckordersid = 0;
	$loadContent = XN_Content::load($record,"ma_inventorybackout");
	...
	$receipt_id=$loadContent->my->receipt_id;//购货者,出库方
	...
	$ing_details=XN_Query::create("Content")
		->tag("ma_returnborrow_details")
		->filter("type","eic","ma_returnborrow_details")
		->filter("my.ma_borrowordersin","=",$ma_borrowordersin)
		->filter("my.returnstatus","=","1")
		->filter("my.deleted","=","0")
		->end(-1)
		->execute();
	if(count($ing_details)>0){
		foreach($ing_details as $detail_info){
			$detail_info->my->returnstatus='2';
		}
		XN_Content::batchsave($ing_details,"ma_returnborrow_details");
	}
	...
	foreach($pickorders as $pickorder_info){
		...
		if(count($query)){
			...
			$recheck_order->save("ma_recheckordersback");
			$recheckordersid=$recheck_order->id;
		}
		else
		{
			$recheck_order=XN_Content::create("ma_recheckordersback","",false);
			...
			$recheck_order->my->ma_inventorybackout=$record;
	...
	$certificateout->my->supplierid=$loadContent->my->receipt_id;
	...
	$certificateout->my->ma_inventorybackout=$record;
	...
	$backout_details=XN_Query::create("Content")
		->tag("ma_backoutdetails")
		->filter("type","eic","ma_backoutdetails")
		->filter("my.record","=",$record)
		->filter("my.deleted","=","0")
		->execute();
	$factorys_names=array();
	foreach($backout_details as $backout_detail){
		$backout_detail->my->submitnumber+=$product_submitnumbers[$backout_detail->my->ma_products];
		$factorys_names[$backout_detail->my->factorys]=$backout_detail->my->factorys_name;
		$backout_detail->save("ma_backoutdetails");
	}
	...
	foreach($pickdetails as $detail_info){
		...
		foreach($batch_infos as $index=>$number_arr){
			...
			$billwaterContent->my->ma_inventorybackout=$record;
			...
			$billwaterContent->my->ma_storagelibs=$Storage_info->my->ma_storagelibs;
			$billwaterContent->my->ma_storageracks=$Storage_info->my->ma_storageracks;
