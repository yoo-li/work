        ...
        <form class="mui-input-group">
            ...
            {foreach name=blocks key=header item=data from=$incheckstorage}
                <div class="mui-collapse-content products" style="margin: 3px 3px;">
                    ...
                    <input style="font-size: 15px; width: 100px" type="text" readonly="readonly" id="barcode_{$header}"
                           value="{$data.barcode}">
                    <input style="font-size: 15px; width: 180px" type="text" readonly="readonly" id="productname_{$header}"
...
//        JsonResult("010694950157155513150508211409277040");
    ...
    function JsonResult(result) {
        ...
        if (checks(itemcodes, sanncode_json)) {
            ...
            var mytestdiv = creatdialog(js_arr, itemcodes, batchcodes, sanncode_json, productdatearr, sterilizecodearr, sterilizevalidatearr);
            ...
            if (document.getElementById('productdate_' + itemcodes + '_' + batchcodes)) {
                productdatearr.put(result, document.getElementById('productdate_' + itemcodes + '_' + batchcodes).value);
            }
            if (document.getElementById('sterilizecode_' + itemcodes + '_' + batchcodes)) {
                sterilizecodearr.put(result, document.getElementById('sterilizecode_' + itemcodes + '_' + batchcodes).value);
            }
            if (document.getElementById('sterilizevalidate_' + itemcodes + '_' + batchcodes)) {
                sterilizevalidatearr.put(result, document.getElementById('sterilizevalidate_' + itemcodes + '_' + batchcodes).value);
            }
    ...
    function newchanges(obj) {
        if (obj.id.indexOf('productnum') == 0) {
            ...
            if (checkRate(obj.id, sanncode_json[hidden]["number"])) {
            ...
            var mydate = obj.value;
            var myproductdate = mydate.substring(0, 4) + "-" + mydate.substring(4, 6) + "-" + mydate.substring(6, 8);
            sterilizevalidatearr.put(hidden, myproductdate);
        }
    }
