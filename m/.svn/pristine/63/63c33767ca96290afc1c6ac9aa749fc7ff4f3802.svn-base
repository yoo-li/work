...
<div class="mui-inner-wrap">
	...
	<div id="pullrefresh" class="mui-content mui-scroll-wrapper" style="padding-top: 85px;">
		<div class="mui-scroll">
					...
					<ul class="mui-table-view" style="padding-top: 5px;padding-bottom: 5px;">
						...
						{*<div class="mui-input-row" style="margin-top:3px;">*}
							{*<div class="numbering2">*}
								{*<label>上级授权编号:</label>*}
								{*<input   name="mall_officialauthorizedimensions_no"*}
									   {*value="" type="text"*}
									   {*class="mui-input-clear" id="mall_officialauthorizedimensions_no" placeholder="5427321">*}
							{*</div>*}
						{*</div>*}
						{*上级授权样式更改*}
			...
			{if $authorizedetail neq ''}
				<div class="mui-input-row authorise_detail" style="margin-top:3px;">
					<label style="height:45px;">授权详情</label>
					<p><span>{$authorizedetail.authorization_person}</span>授权<span>{$authorizedetail.authorized_person}（决定人）</span>对<span
						>{$authorizedetail.applicant}</span
						>提出的<span
						>授权事项（授权类别）</span>形式决定权。</p>
					<p><span>被授权人（决定人）{$authorizedetail.authorized_person}</span>在做决定前应征询<span>关注人
							{$authorizedetail.opinion}</span>意见。</p>
					<p>在该授权范围的业务申请应抄送<span>关注人 {$authorizedetail.opinion}</span>。</p>
					<p>授权有效期从<span>{$authorizedetail.validity}</span>到<span>{$authorizedetail.expiry}</span>止。</p>
				</div>
				<input id="authorizedperson" name="authorizedperson" value="{$authorizedetail.authorization_person}" type="hidden">{*授权人*}
				<input   name="authorizeperson_name" value="{$authorizedetail.authorization_person_name}" type="hidden">{*授权人*}

				<input id="applicant" name="applicant" value="{$authorizedetail.applicant}" type="hidden">
				<input id="opinion" name="opinion" value="{$authorizedetail.opinion_name}" type="hidden">{*关注人*}
				<input  name="opinion_name" value="{$authorizedetail.opinion_name}" type="hidden">{*关注人*}

				<input id="validity" name="validity" value="{$authorizedetail.validity}" type="hidden">
				<input id="validity" name="expiry" value="{$authorizedetail.expiry}" type="hidden">
				<input id="authorization_person" name="copy" value="{$authorizedetail.authorized_person_name}" type="hidden"> {*决定人*}
				<input  name="authorized_person_name" value="{$authorizedetail.authorized_person_name}" type="hidden"> {*决定人*}
...
<script type="text/javascript">
	var decider = "{$authorizedetail.authorized_person_name}";
	var validity =  "{$authorizedetail.validity}";
	var expiry = "{$authorizedetail.expiry}"; mall_officialauthorizedimensions_no
    var users = {$users};
    var officialauthorizedimensions = {$officialauthorizedimensions};
    var authorizedimensions = {$authorizedimensions};

    var	users = {$users} //添加父授权事件
    {literal}
    var simulatedData = [];
    mui.init({
        pullRefresh: {
