...
//    XN_REST::$LOCAL_API_HOST_PORT               = $_SERVER['HTTP_X_NING_LOCAL_API_HOST_PORT'];
    ...
    $ma_inventorybackout     = $instorage_info->my->ma_inventorybackout;
    ...
    $returnordersin_info          = XN_Content::load($ma_inventorybackout, "ma_inventorybackout");
    ...
    else
    {
        ...
        $checkorder_info->my->ma_inventorybackout     = $ma_inventorybackout;
        ...
        $saledetails          = XN_Query::create("Content")
            ...
            ->filter("my.record", "=", $ma_inventorybackout)
            ->filter("my.deleted", "=", "0")
            ->execute();
        foreach ($saledetails as $detail_info)
        {
            ...
            $checkdetail->my->ma_inventorybackout     = $ma_inventorybackout;
    ...
    {
        //减法
        $smarty->display('sa_backincheckstoragebysub.tpl');
    }
    else
    {
        //加法
