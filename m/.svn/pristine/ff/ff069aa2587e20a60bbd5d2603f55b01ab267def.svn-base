...
<div id="pullrefresh" class="mui-content mui-scroll-wrapper" style="padding-top: 50px;">
    <div class="mui-collapse-content">
        <form class="mui-input-group">
            ...
            {foreach name=blocks key=header item=data from=$returnoutinfo}
                    ...
                    {foreach  key=key item=info from=$data.batch_info}
                        ...
                        <div class="mui-input-row">
                            ...
                            <div class="mui-numbox" data-numbox-min='0' data-numbox-max='9999999999' style="width:170px">
                                ...
                                {if $data.suppliername eq '医流通'}
                                    <input id="productnum_{$data.barcode}_{$info.products_batch_no}" class="mui-input-numbox" type="number"
                                           value="{$info.submitnumber}" onchange="changes(this);"/>
                                {else}
                                    <input id="productnum_{$data.itemcode}_{$info.products_batch_no}" class="mui-input-numbox" type="number"
                                           value="{$info.submitnumber}" onchange="changes(this);"/>
                                {/if}
                        ...
                        {if $data.suppliername eq '医流通'}
                            <input id="hidden_productnum_{$data.barcode}_{$info.products_batch_no}" class="mui-input-numbox" type="hidden"
                                   value="{$info.submitnumber}"/>
                        {else}
                            <input id="hidden_productnum_{$data.itemcode}_{$info.products_batch_no}" class="mui-input-numbox" type="hidden"
                                   value="{$info.submitnumber}"/>
                        {/if}
                    {/foreach}
                </div>
...
<script type="text/javascript" charset="utf-8">
    ...
    mui('body').on('tap', 'button#finishsann', function (e) {
        ...
        for (var i = 0; i < mybarcodes.length; i++) {
            ...
            myjson.number = parseInt(document.getElementById("hidden_productnum_" + itemcodes + "_" + batchcodes).value) - parseInt(btnnum);//已复核数量
            myjson.sendnumber = parseInt(document.getElementById("hidden_productnum_" + itemcodes + "_" + batchcodes).value);//要复核的数量
            myarr.push(myjson);
        }
        ...
        mui.ajax({
            ...
            traditional: true,
            dataType: "json",
            url: "save_recheckordersout.php",
            data: {"mybarcodes": fields, "record": record},
    ...
    function getdata(obj) {
        ...
        mui.ajax({
            ...
            dataType: "json",
            url: "find.php",
            data: {"record": record, "barcode": obj.value, "mode": "recheckordersout"},
            success: function (data) {
