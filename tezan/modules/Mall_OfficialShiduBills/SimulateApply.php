    ...
    isset($_REQUEST['type']) && $_REQUEST['type'] == "submit" )
{
    try {
		...
		$loadcontent->my->$status = 'Submit';
		$loadcontent->my->finishapprover = XN_Profile::$VIEWER;
		$loadcontent->my->submitapprovalreplydatetime = date("Y-m-d H:i"); 
		 
		$loadcontent->save(strtolower($currentModule).','.strtolower($currentModule).'_'.$businesseid);  
		
		global $currentModule;
		$profileid = $loadcontent->my->profileid;
		$authorizedenterprisecurrency = $loadcontent->my->authorizedenterprisecurrency;
		$enterprisecurrencyid = $loadcontent->my->enterprisecurrencyid;
		if (floatval($authorizedenterprisecurrency) > 0)
		{
			...
			$newcontent->my->enterprisecurrencytype = 'newprofile';  
			$newcontent->my->money = "0.00";
			$newcontent->my->amount = number_format($authorizedenterprisecurrency,2,".","");
