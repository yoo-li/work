...
$field_query = XN_Query::create ( 'Content' )->tag ( 'Fields' )
	      ...
	      ->filter ( 'my.uitype', '!in', array('4','70') )
...
function getFieldListEntries($module)
{
	...
	foreach($blocks as $block_info){
			...
			$field_query = XN_Query::create ( 'Content' )->tag ( 'Fields' )
				      ...
				      ->filter ( 'my.uitype', '!in', $nonEditableUiTypes )
				      ->order('my.sequence',XN_Order::ASC_NUMBER);
			...
			if (count($fields) > 0)
			{
				...
				$cf_hidden_element=Array();
			 	...
			 	foreach($fields as $field_info){
					...
					$customfieldflag = $field_info->my->iscustom;
					...
					$visibility = getFieldInfo($fieldname,$typeofdata,$presence,$strictlyMandatory,$customfieldflag,$displaytype,$uitype);
					...
					if ($presence == 0 || $presence == 2) {
						...
						$cf_element[$count]['editwidth']=$field_info->my->editwidth;
						...
						$cf_hidden_element[$count]['width']=$field_info->my->width;	
						$cf_hidden_element[$count]['editwidth']=$field_info->my->editwidth;
						$cf_hidden_element[$count]['align']=$field_info->my->align;				
 ...
 * 
 */
function getFieldInfo($fieldname,$typeofdata,$presence,$strictlyMandatory,$customfieldflag,$displaytype,$uitype){
...
function updateFieldProperties(){
	...
	$fields = XN_Query::create ( 'Content' )->tag ( 'Fields' )
					      ...
					      ->filter ( 'my.fieldname', '!=', 'salutationtype' )
					      ->filter ( 'my.presence', 'in', array('0','2') )
					      ->execute();
	...
	if (count($fields) >0 ){
			...
			$old_display_editwidth = $field_info->my->editwidth;
			...
			$display_editwidth = $_REQUEST['display_editwidth'];
			...
			if(!empty($qcdata)){
				$field_info->my->quickcreate = $qcdata;
				$field_info->my->quickcreatesequence = $maxseq+1;
			}
			...
			$field_info->my->editwidth = $display_editwidth;
