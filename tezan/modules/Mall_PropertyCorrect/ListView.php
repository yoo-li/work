...
//<<<<<<<<customview>>>>>>>>>
    ...
    //删除掉那些只创建不保存的
    $cancelContents=XN_Query::create ( 'Content' ) ->tag($focus->table_name)
        ->filter ( 'type', 'eic', $focus->table_name)
        ->filter ( 'my.deleted',"=",'-1')
        ->end(-1)
        ->execute();
    if(count($cancelContents) > 0)
    {
        foreach ( array_chunk($cancelContents,50,true) as $chunk_fields)
        {
			$productids = array();
			foreach($chunk_fields as $chunk_field_info)
			{
				$productids[] = $chunk_field_info->id;
			}
