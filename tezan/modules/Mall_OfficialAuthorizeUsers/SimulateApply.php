    ...
    isset($_REQUEST['type']) && $_REQUEST['type'] == "submit" )
{
    try {
		...
		$homepage = $loadcontent->my->homepage;
		if (!isset($homepage) || $homepage == "")
		{
			echo '{"statusCode":"300","message":"活动广告图不能为空！"}';
			die();
		}
		...
		$loadcontent->save("mall_salesactivitys,mall_salesactivitys_".$supplierid);  
		
		
	    $activity_products=XN_Query::create("Content")
	        ->tag("mall_salesactivitys_products_".$supplierid)
	        ->filter("type","eic","mall_salesactivitys_products")
	        ->filter("my.salesactivityid","=",$record)
	        ->end(-1)
	        ->execute();
	    foreach($activity_products as $apinfo){
	        $apinfo->my->status = '0';
			$apinfo->my->approvalstatus = '2';
	        $apinfo->save("mall_salesactivitys_products,mall_salesactivitys_products_".$supplierid);
	    }
