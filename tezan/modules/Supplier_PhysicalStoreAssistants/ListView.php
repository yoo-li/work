...
{
	if (!isset($_REQUEST['physicalstore']))
	{
		if (isset($_SESSION['PHYSICALSTOREID']) && $_SESSION['PHYSICALSTOREID'] != "")
		{
			$_REQUEST['physicalstore'] = $_SESSION['PHYSICALSTOREID'];
		}
	} 
}
...
foreach($supplier_physicalstores as $supplier_physicalstore_info)
{
    ...
    if($_REQUEST['physicalstore']==$physicalstoreid)
	{
        $categoryOption .= '<option value='.$physicalstoreid.' selected="selected">'.$storename.'</option>';
    }
	else
	{
...
$searchpanel["实体店"]='<div style="display:block;float:right;width:200px;margin-top:-5px;">
		<select data-toggle="selectpicker" id="physicalstore" name="physicalstore"   style="cursor: pointer;width:180px;">'.$categoryOption.'</select>
        </div>';
...
if (isset($_REQUEST['physicalstore']) && $_REQUEST['physicalstore'] != "")
{
	$query->filter ( 'my.physicalstoreid', '=', $_REQUEST['physicalstore']);
	$_SESSION['PHYSICALSTOREID'] = $_REQUEST['physicalstore'];	 
}
