...
if (check_authorize('tezanadmin') || is_admin($current_user)){
    $searchpanel["供应商"]='<div style="display:inline-block;width:200px;">
		<input id="suppliers_search_input" class="search-text textInput" type="text" placeholder="输入供应商名称" name="suppliers_search_input" value="'.$_REQUEST["suppliers_search_input"].'" style="width:180px;">
		</div>';
}
...
if(isset($_REQUEST["orders_search_input"]) && $_REQUEST["orders_search_input"] != ""){
    ...
    ->order('my.orders_no',XN_Order::DESC)
    ->execute();
    $ids = array();
...
if(count($statistics)>0)
    $smarty->assign("CUSTOMSHOWMSG", "<font color=blue>&nbsp;&nbsp;&nbsp;&nbsp;应付金额合计：￥".$statistics[0]->my->amount . "&nbsp;&nbsp;&nbsp;&nbsp;实付金额合计：￥" . $statistics[0]->my->total_fee ."</font>");
*/
