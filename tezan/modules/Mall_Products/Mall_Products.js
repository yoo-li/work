...
function mall_productlibs_callback(groupid,args)
{
    jQuery.post("index.php?module=Mall_Products&action=CopyFromLibs&type=submit&record="+args.id,'',function(data){  
		 $(this).navtab("refresh");   
    });
}
...
function confirm_select_mall_products(){
    ...
    var names="";
    var shop_price="";
	$.CurrentDialog.find("#refresh_listview_entries").find("div>table>tbody>tr>td>input[name='ids']:checked").each(function(){
        ids+=$(this).val()+";";
		names+=$(this).parent().parent().find("td.productname").text()+";";
        shop_price+=$(this).parent().parent().find("td.shop_price>span").text()+";";
    })
    ids=ids.substr(0,ids.length-1);
    names=names.substr(0,names.length-1);
    shop_price=shop_price.substr(0,shop_price.length-1);
	var callback=$("#callback").val();
	var obj={"id":ids,"Name":names,"Shop_Price":shop_price};
	//alert('"'+callback+"({\"id\":"+ids+",\"Name\":"+names+",\"Shop_Price\":"+shop_price+"})"+'"');
	eval('"'+callback+"("+obj+")"+'"');
	//return callback({"id":ids,"Name":names,"Shop_Price":shop_price});
}
*/
