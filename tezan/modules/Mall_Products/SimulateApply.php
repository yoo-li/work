    ...
    isset($_REQUEST['type']) && $_REQUEST['type'] == "submit" )
{
    try {
	    ...
	    $productthumbnail = $loadcontent->my->productthumbnail;
	 
	    $productlogo = $loadcontent->my->productlogo;
		
		if (!isset($productthumbnail) || $productthumbnail == "")
		{
			echo '{"statusCode":"300","message":"商品的缩略图不能为空！"}';
			die();
		}
		
		if (!isset($productlogo) || $productlogo == "")
		{
			echo '{"statusCode":"300","message":"商品的广告图不能为空！"}';
			die();
		}
