...
if($focus->column_fields[strtolower($currentModule). 'status'] == '' || $focus->mode == "create")
{
    ...
    $focus->column_fields['hitshelf'] = 'off';
	$focus->column_fields['sequence'] = '1000';
	$focus->column_fields['profitrate'] = '0';
	
}

$focus->column_fields["datafrom"] = '后台';
$focus->column_fields['commissionswitch'] = '0';
...
if($focus->column_fields['uniquesale'] == '' )
{
	$focus->column_fields['uniquesale'] = '0';  
}
...
if(isset($focus->column_fields['distributionstatus']) && $focus->column_fields['distributionstatus'] == '' )
{ 
	$focus->column_fields['distributionstatus'] = '0'; 
}

$focus->column_fields['preview'] = '/web/previewdetail.php';
try {
    if($newq != ""){
        $focus->column_fields['qualitycertificate'] = $newq;
    }else{
        $focus->column_fields['qualitycertificate'] = array();
    }
	
    if(isset($_SESSION['vendorid']) && $_SESSION['vendorid'] != '')
    {
        $vendorid = $_SESSION['vendorid'];
		$vendor_price = $focus->column_fields['shop_price'];
		$focus->column_fields['vendorid'] = $vendorid;
		$focus->column_fields['vendor_price'] = $vendor_price;
		$record = $_REQUEST['record'];
		...
		if (count($mall_product_property) > 0)
		{
			...
			{ 
					$vendor_price = $mall_product_property_info->my->shop; 
	...
	{
		$focus->column_fields['vendorid'] = "";
		$focus->column_fields['vendor_price'] = "";
	}
...
echo '{"statusCode":"200","message":null,"tabid":"'.$currentModule.'","closeCurrent":"false","forward":"/index.php?module='.$currentModule.'&action=EditView&record='.$focus->id.'"}';
