...
{
    include ('modules/Mall_Orders/SheetExportExcel.php');
	die();
}
...
if (isset($supplierid) && $supplierid != "")
{
	try{
		...
		{
			 $focus->special_search_fields['order_status' ] = array(
...
$searchpanel["会员"]='<input id="profile_search_input" class="search-text textInput" type="text" placeholder="输入会员昵称" name="profile_search_input" value="'.$_REQUEST["profile_search_input"].'" style="width:180px;">';
...
$statistics = XN_Query::create ( 'YearContent_Count' )->tag($focus->table_name)
	...
	->rollup('my.usemoney')
    ->rollup('my.paymentamount');

if(isset($_REQUEST['order_status']) && $_REQUEST['order_status'] == 'trade'){
	$_REQUEST['order_status'] = "";
	$query->filter ( 'my.tradestatus', '=', 'trade');
	$statistics->filter ( 'my.tradestatus', '=', 'trade');  
}
...
if(isset($_REQUEST['profile_search_input']) && trim($_REQUEST['profile_search_input'])!=""){
    $profile_search_input=$_REQUEST['profile_search_input']; 
	$profile = XN_Query::create ( 'Profile' ) ->tag("profile")
		->filter('type','=','wxuser')
		->filter('givenname','like',$profile_search_input)
		->begin(0)->end(200)
		->execute();
	$pids = array();
