    ...
    isset($_REQUEST['type']) && $_REQUEST['type'] == "submit")
{
    try {
        ...
        array_unique($binds);
        $salesactivitys=XN_Content::loadMany($binds,"Mall_RobSingles_details");
        ...
        foreach($salesactivitys as $activity_info){
            ...
            $activity_info->save("Mall_RobSingles_details,Mall_RobSingles_details_".$supplierid);
            ...
            foreach($activity_products as $info){
                ...
                $info->save("Mall_RobSingles_details,Mall_RobSingles_details_".$supplierid);
            }
        }
    ...
    $enddate=date("Y-m-d");
    if(count($binds)==1){
        $content=XN_Content::load($ids,"Mall_RobSingles_details");
    ...
    $smarty->assign("SUBMODULE", "Mall_RobSingles");
