...
if(isset($_REQUEST['record']) && $_REQUEST['record'] !='') 
{
	...
	$correctcontent = XN_Content::load($_REQUEST['record'],strtolower($currentModule));
    $correctlongitude = $correctcontent->my->correctlongitude;
	$correctlatitude = $correctcontent->my->correctlatitude;
    if($suppliercorrectContent->my->record){
        ...
        $suppliers=XN_Query::create("Content")
            ...
            ->end(1)
            ->execute();
        $supplierContent=$suppliers[0];
        $latitude = $supplierContent->my->latitude;
        $longitude = $supplierContent->my->longitude;
    }

    if(!$correctlongitude && !$correctlatitude){
	...
	$lowermodule =  strtolower($currentModule);
		...
		echo '
				...
				<input id="correct_baidumap_longitude" name="correctlongitude" type="hidden" value="'.$correctlongitude.'"/>
				<input id="correct_baidumap_latitude" name="correctlatitude" type="hidden" value="'.$correctlatitude.'"/>
