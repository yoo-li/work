...
function get_supplier_info($supplierid = null)
{
    ...
    if (isset($supplierinfo) && count($supplierinfo) > 0 && !empty($supplierinfo))
    {
        return $supplierinfo;
    }
    ...
    if (count($supplier_settings) > 0)
    {
		...
		//$supplierinfo['popularizeqrcode']  = $supplier_setting_info->my->popularizeqrcode; 
    ...
    if (count($mall_settings) > 0)
    {
		...
		$realnameauthentication = $mall_setting_info->my->realnameauthentication;
		$presalesconsultation = $mall_setting_info->my->presalesconsultation;
		$productappraises = $mall_setting_info->my->productappraises;
		$authenticationpayment = $mall_setting_info->my->authenticationpayment;
		$authenticationpayment = $mall_setting_info->my->authenticationpayment;
        ...
        if (!isset($realnameauthentication) || $realnameauthentication == "")
        {
            $realnameauthentication = '1';
        } 
		...
		$supplierinfo['realnameauthentication']  = $realnameauthentication;
