...
class Uploader
{
    ...
    private function upFile()
    {
		...
		$hashval=md5_file($file["tmp_name"]);
		...
		if (count($attachments) > 0)
		{
	        ...
	        $this->fullName = $upload_file_path.$savefile; 
		...
		else
		{
	        ...
	        if (!(move_uploaded_file($file["tmp_name"], $this->filePath) && file_exists($this->filePath))) { //移动失败
				...
				$mime_type = $this->mime_type($this->filePath); 
		        $upload_file_path = dirname($this->fullName);
		        $Attachments = XN_Content::create('attachments','',false)
		            ->my->add('name',$this->oriName)
		            ->my->add('module','ueditor')
		            ->my->add('record','0')
		            ->my->add('filesize',$this->fileSize)
		            ->my->add('type',$mime_type)
		            ->my->add('path',$upload_file_path.'/')
		            ->my->add('savefile',$this->fileName)
		            ->my->add('sequence','0')
