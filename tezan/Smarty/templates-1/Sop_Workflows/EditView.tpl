...
<div class="bjui-pageContent tableContent" style="overflow:hidden;">
	<form id="salesEditViewForm" method="post" action="index.php" callback="{$MODULE|@strtolower}_validate" data-toggle="validate" data-validator-option="{ldelim}focusCleanup:true,timely:false{rdelim}" data-alertmsg="false">
								...
								<div id="{$topdata.action|lower}_form_div">
									<script language="javascript" type="text/javascript">
										...
										{if $topdata.params neq ''}
											{if $topdata.params|is_array}
												{foreach item=subparam from=$topdata.params}
													params += "&{$subparam}="+$("#{$subparam}").val();
												{/foreach}
											{else}
											...
											<script language="javascript" type="text/javascript">
												...
												{if $data.params neq ''}
													{if $data.params|is_array}
														{foreach item=subparam from=$data.params}
															params += "&{$subparam}="+$("#{$subparam}").val();
														{/foreach}
													{else}
								...
								<div class="panel panel-default" style="margin:2px;">
									<div class="panel-heading">
										<h3 class="panel-title">{$data.action|@getTranslatedString:$MODULE}
											<a style="float:right;cursor:pointer;text-decoration: none;font-size:17px;"  data-toggle="collapse" data-target="#{$data.action|lower}_form_div"><i class="fa btn-default fa-caret-square-o-up"></i><i class="fa btn-default fa-caret-square-o-down"></i></a>
										</h3>
									</div>
									<div style="padding:0;" class="panel-body bjui-doc">
										<div id="{$data.action|lower}_form_div"  class="collapse in">
											<script language="javascript" type="text/javascript">
												...
												{elseif $data.action eq 'FlowFields' || $data.action eq 'FlowAttachments'}
													jQuery("#{$data.action|lower}_form_div").attr("data-toggle","autoajaxload");
													jQuery("#{$data.action|lower}_form_div").attr("data-url",postBody);
													jQuery("#{$data.action|lower}_form_div").removeClass("in");
												{else}
						...
						<div class="tab-pane navtab-panel tabsPageContent" id="{$paneldata.action|lower}_form_div">
							<script language="javascript" type="text/javascript">
								...
								{if $paneldata.params neq ''}
									{if $paneldata.params|is_array}
										{foreach item=subparam from=$paneldata.params}
											params += "&{$subparam}="+$("#{$subparam}").val();
										{/foreach}
									{else}
...
<div class="bjui-pageFooter" {if $RUNTIME neq ''}title="响应时间:{$RUNTIME}s"{/if}>
    <ul>
		...
		{if $MODE eq 'edit'}
		<li><a class="btn btn-green" data-icon="save" {if $READONLY eq 'true'}disabled{/if} onclick="SaveWorkflows({$APPROVALSTATUS});" href="javascript:void(0);">{$APP.LBL_SAVE_BUTTON_LABEL}</a></li>
		{else}
		<li><button type="submit" id="savebutton" {if $READONLY eq 'true'}disabled{/if} class="btn-green" data-icon="save">{$APP.LBL_SAVE_BUTTON_LABEL}</button></li>
		{/if}
