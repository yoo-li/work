...
<div class="bjui-pageFooter" {if $RUNTIME neq ''}title="响应时间:{$RUNTIME}s"{/if}>
    <ul>
        <li><button type="button" class="btn-close" data-icon="close">关闭 </button></li>{*//授权事件关闭*}

		<li><button type="submit" id="savebutton" {if $READONLY eq 'true'}disabled{/if} class="btn-green" data-icon="save">{$APP.LBL_SAVE_BUTTON_LABEL} </button></li>{*授权事件保存*}
		...
		{if $HASAPPROVALS eq 'true' && $MODE eq 'edit'}
			{if $READONLY eq 'true' || $read_only eq '1' || $APPROVALSTATUS == '1' || $APPROVALSTATUS == '2'}
				...
				{/if}
			{else}
				<li> <a id="submitapprovals_btn" class="btn btn-default" data-icon="envelope" onclick="submitApprovals();" title="{$APP.LBL_APPROVALS_BUTTON_LABEL}" href="#">44{$APP.LBL_APPROVALS_BUTTON_LABEL}</a></li>
			{/if}
		{/if}
    </ul>
</div>
...
//        alert('hello');
//        alert('index.php?module=Approvals&action=viewApprove&record={$ID}&mode=submit&formodule={$MODULE}&from=editview');
    })
//
//    )
