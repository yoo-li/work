...
table.inner-table>tbody>tr>td{height:30px;overflow: hidden;}
table.inner-table>tbody>tr{border:none;}
{/literal}
</style>
{if $MSG}{$MSG}{/if}
				...
				<li>
					...
					<div class="pull-right" style="margin-right:14px;margin-top: -4px;">
						<script type="text/javascript">
							...
							{ldelim}
								var viewid = $('#{$MODULE|@strtolower}_viewname').val();
								$('#{$MODULE|@strtolower}_viewid').val(viewid);
...
{if $ZTREEDATA eq ''}
	<div id="refresh_listview_entries" class="bjui-pageContent tableContent bjui-resizeGrid">
		{include file="Ma_PaymentList/ListViewEntries.tpl"}
	</div>
{else}
