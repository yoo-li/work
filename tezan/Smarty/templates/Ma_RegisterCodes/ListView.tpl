					...
					<tr>
						...
						<td style="padding-right:14px;width:78px;vertical-align:bottom;">
							<script type="text/javascript">
								var module="{$MODULE}";
								var tree_listview_div_id=module+"_tree-listview";
								{literal}
								...
								function ajaxsearchFrame(){
									if($.CurrentNavtab.find("#"+tree_listview_div_id)){
										var paramstr = getPostParams();
										var postBody = "index.php?mode=ajax&"+paramstr;
										$.CurrentNavtab.find("#"+tree_listview_div_id).ajaxUrl({url:postBody, loadingmask:true})
									}
								}
							{/literal}
							</script>
...
{if $ZTREEDATA eq ''}
	<div id="{$MODULE}_tree-listview" class="bjui-pageContent tableContent">
		{include file="ListViewEntries.tpl"}
	</div>
{else}
	<div id="{$MODULE}_tree-listview" class="bjui-pageContent tableContent">
		<div id="{$MODULE}_tree_div" class="bjui-pageContent tableContent tree-left-box" style="width:30%;">
			{$ZTREEDATA}
		</div>
		<div id="{$MODULE}_listview_div" class="bjui-pageContent tableContent" style="left: 30%;width:70%;">
			{include file="ListViewEntries.tpl"}
		</div>
	</div>
{/if}
