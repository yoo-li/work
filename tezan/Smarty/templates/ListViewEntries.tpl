			...
			<tr data-id="{$entity_id}">
				...
				{foreach item=data key=key from=$entity}
					{foreach name="listviewforeach" item=header key=headkey from=$LISTHEADER}
						{if $headkey eq $key }
						    {if $key eq 'oper' || $key eq 'record' || $header.notip eq 1}
	...
	<div class="pages">
		...
		<div class="selectPagesize">
			<select data-toggle="selectpicker" data-toggle-change="changepagesize">
				...
				<option value="500" {if $NUMPERPAGE eq '500'}selected{/if}>500</option>
				<option value="1000" {if $NUMPERPAGE eq '1000'}selected{/if}>1000</option>
			</select>
		</div>
