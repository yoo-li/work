...
{/if} 
	...
	<table data-toggle="tablefixed" class="table table-striped table-bordered table-hover nowrap tree" data-width="100%" data-nowrap="true">
		<thead>
		<tr>
		...
		{foreach name="listviewentity" item=entity key=entity_id from=$LISTENTITY}
			<tr data-id="{$entity_id}" class="{$entity.treegrid}">
				<td class="center" width="30px" maxwidth="20"><input type="checkbox" name="ids" data-toggle="icheck" value="{$entity_id}"></td>
				{foreach item=data key=key from=$entity}
					{foreach name="listviewforeach" item=header key=headkey from=$LISTHEADER}
						{if $headkey eq $key }
						    {if $key eq 'oper' || $header.notip eq 1}
		...
		<tr>
			...
			<td class="center" colspan="{$LISTHEADER|@count}" style="padding:5px;padding-top:15px;background-color:#fff;">
				<div style="border:1px solid #dfdfdf;background:#fff6e1;">
					...
					<tr>
						<td class="center" width="20" rowspan="2" align="center" valign="top"><img src="/Public/images/x1chart.png"></td>
