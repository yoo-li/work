<?php
	...
	$smarty->assign("POPUPTYPE", $currentModule);

	$smarty->assign("HASAPPROVALS", $hasapprovals);
	$smarty->assign("APPROVALSTATUS", $approvalstatus);

	$status = strtolower($currentModule).'status';
	if ($focus->column_fields[$status] == 'Archive' || $focus->column_fields[$status] == 'Terminate')
		$readOnly = 'true';

	if ($focus->column_fields["deleted"] == '1')
		$readOnly = 'true';
...
// Module Sequence Numbering
	...
	$ModuleStatus = strtolower($currentModule."status");
	if (isset($focus->column_fields[$ModuleStatus]))
	{
		$smarty->assign("MODULESTATUS", $focus->column_fields[$ModuleStatus]);
	}
