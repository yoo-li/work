    ...
    isset($_REQUEST['type']) && $_REQUEST['type'] == "submit"  )
{
		...
		if (count($binds) > 0)
		{
	        ...
	        {  
				...
				if (count($supplier_wxsettings) > 0)
				{
					...
					$message = '您好,您提交的提现已经驳回，请前往【提现申请】-【历史提现记录】界面查看原因，再重新提交！';
	...
	else
	{
		...
		if (isset($takecashsstatus) && $takecashsstatus != "待处理")
		{
			$smarty = new vtigerCRM_Smarty;
			global $mod_strings;
			global $app_strings;
			...
			$smarty->assign("APP", $app_strings);
			$smarty->assign("CMOD", $mod_strings); 
		    $msg = '<div style="width:100%;text-align:center;"><br><br><font color="red" size="2">只能对“待处理”的申请进行驳回操作!</font></div>';  
			$smarty->assign("MSG", $msg);  
			$smarty->display("MessageBox.tpl");
			die();
		} 
	...
	$msg =  '<div class="form-group">
	                 <label class="control-label x120">驳回原因:</label>
					 <textarea data-rule="required"  style="width:200px;height:50px;" name="rejectreason" class="required"/></textarea> 
			 </div> ';
}
