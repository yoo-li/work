<?php
	...
	if ($focus->column_fields['physicaltype'] == "1")
	{
		...
		$status             = XN_Filter("my.status", '=', "Active");
		$filter             = array ();
		$filter['delivery'] = base64_encode(serialize(XN_Filter::all($logisticsname, $status)));
	}
	else
	{
		$status             = XN_Filter("my.status", '=', "Active");
		$filter             = array ();
		$filter['delivery'] = base64_encode(serialize(XN_Filter::all($status)));
	}
...
//$smarty->display("modules/Orders/EditView.tpl");
?>