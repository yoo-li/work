...
global  $supplierid,$supplierusertype,$accesstype;
if ($supplierusertype != "superadmin")
{ 
    $query ->filter('my.supplierid', '=', $supplierid);
    if (isset($supplierid) && $supplierid != "" && $supplierid != "0")
    {
        $systemfilter1 = XN_Filter::all(XN_Filter( "author", '=',XN_Profile::$VIEWER));
        $systemfilter2 = XN_Filter::all(XN_Filter( "author", '<>',XN_Profile::$VIEWER),XN_Filter( "my.announcementsstatus", '=','Release'));
        $customfilter = XN_Filter::any($systemfilter1,$systemfilter2);
...
//    $customfilter = XN_Filter::any(XN_Filter("my.announcementslocation", '=', "On Top"), XN_Filter("my.recipient", '=', XN_Profile::$VIEWER));
//    $permission   = listviewpermission($currentModule, $search, $customfilter);
