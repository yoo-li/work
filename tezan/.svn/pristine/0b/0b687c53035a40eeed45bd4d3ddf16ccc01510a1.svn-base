...
$search->setQuery($query);
    ...
    $detail_query=XN_Query::create("Content")
        ->tag("ma_suppliers")
    ...
    if(count($detail_query) > 0){
        $supplier_ids = $detail_query[0]->id;
	}

//自定义的查询字段，要自己链接到$query查询语句里面
global  $current_user,$supplierusertype,$user_type_id,$supplierid;
if($supplierusertype!="admin" && $supplierusertype!="superadmin" && $supplierusertype!="ma_foodanddrugadmin" )
{
...
$listview_entries=array();
    $modifystatus=array("1"=>"最大库存预警","2"=>"最小库存预警");
    foreach($list_result as $info){
        $url=$info->my->info_name;
        $listview_entries[$info->id]=array(
            "relation_id"=>$url,
            "barcode"=>$info->my->barcode,
            "module_type"=>$modifystatus[$info->my->module_type],
