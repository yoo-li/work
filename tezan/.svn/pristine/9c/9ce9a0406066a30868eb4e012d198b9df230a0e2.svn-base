...
//$sessSavePath = dirname(__FILE__)."/cache/";
...
if (!echo_validate_image($config))
{
	...
	update_securimage_code($sessionID,'abcd');
...
function update_securimage_code($guid, $chcode)
{
	   try {
			XN_Profile::$VIEWER = null;
			XN_MemCache::put($chcode,"checkcode_".$guid,"6000");   
		} 
...
function echo_validate_image( $config = array() )
{
	...
	//$_SESSION[$sessionID] = strtolower($rndstring);
	update_securimage_code($sessionID,strtolower($rndstring));
    ...
    for($i=0;$i<$rndcodelen;$i++)
    {
        ...
        $c = mt_rand(0, 15);
        @imagettftext($im, $font_size, $c, $y_pos+12, 24, $c_fontColor, $font_file, $rndstring[$i]);
        $lastc = $rndstring[$i];
    }
