...
//$query->filter ( XN_Filter::any(XN_Filter('my.suppliers_type', '<=',1),XN_Filter('my.suppliers_type', '=',null)));
if (!check_authorize('tezanadmin') && !is_admin($current_user)){  
	$cur_suppliers=XN_Query::create("Content")
        ->tag("suppliers")
        ->filter("type","eic","suppliers")
        ->filter("my.pers","=",XN_Profile::$VIEWER)
        ->filter("my.deleted","=","0")
        ->end(1)->execute();
    if(count($cur_suppliers)){
        $cur_supplier_info=$cur_suppliers[0];
        $cur_supplierid=$cur_supplier_info->id;
        if($cur_supplier_info->my->suppliers_type=='0' || $cur_supplier_info->my->suppliers_type=='' || $cur_supplier_info->my->suppliers_type=='1' ||$cur_supplier_info->my->suppliers_type=='2' ){
            $cur_supplier=XN_Filter("id","in",array($cur_supplierid));
            $lower_supplier=XN_Filter("my.uppergrade","=",$cur_supplierid);
            $query->filter ( XN_Filter::any($cur_supplier,$lower_supplier));
        }
        if($cur_supplier_info->my->suppliers_type=='3'){
            $lower_suppliers=XN_Query::create("Content")
                ->tag("suppliers")
                ->filter("type","eic","suppliers")
                ->filter("my.uppergrade","=",$cur_supplierid)
                ->filter("my.deleted","=","0")
                ->end(30)->execute();
            $lower_supplier_ids=array();
            if(count($lower_suppliers)){
                foreach($lower_suppliers as $low_info){
                    $lower_supplier_ids[]=$low_info->id;
                }
            }
            $query->filter ( XN_Filter::any(XN_Filter("id","in",$lower_supplier_ids),XN_Filter("my.uppergrade","in",$lower_supplier_ids)));
        }
