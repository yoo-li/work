...
if(isset($_REQUEST['record']) && $_REQUEST['record'] !='') 
{
	...
	if ($longitude != "" && $latitude != "")
	{
       ...
       if(is_numeric($longitude)&&strstr($longitude,'.')){
         $longitude = round($longitude,6);
      	 $latitude =  round($latitude,6);
         //将有小数点的 地图重新保存  
        }
        else{
           $longitude = intval($longitude)/10000000;
		   $latitude = intval($latitude)/10000000;
        }
	...
	//echo '____'.$latitude.'______'.$longitude.'_________<br>';
	//die();
	/*
	$gcj02 = XN_Earth::bd09_conv_gcj02($latitude,$longitude); 
	print_r($gcj02);
	$gcjLat = $gcj02['lat'];
	$gcjLng = $gcj02['lng'];
	...
	echo '____'.$latitude.'______'.$longitude.'_________';
	//die();
    /*$latitude = "324722650";
   // $longitude = "1257836540";
	
    if($latitude == "" || $longitude == ""){ 
        $latitude = "3247531.46";
        $longitude = "12579719.10";
    } 
	else
	{
		$latitude = floatval($latitude)/100; 
		$longitude = floatval($longitude)/100;
	}  
 

	$wgs04 = XN_Earth::bd09mc_conv_wgs04($latitude, $longitude);
