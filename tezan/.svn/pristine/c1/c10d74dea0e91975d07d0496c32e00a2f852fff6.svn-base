...
function wxtype_onchange() 
{
		...
		if (wxtype && wxtype != "")
		{
			if (wxtype == "1")
			{
				$.CurrentNavtab.find('#cid_welcometitle').css("display","none");
				$.CurrentNavtab.find('#cid_image').css("display","none");
				$.CurrentNavtab.find('#welcometitle').removeClass("required");
				$.CurrentNavtab.find('#salesEditViewForm').validator("setField", "welcometitle", "");
				
				$.CurrentNavtab.find('#image_crop_avatar').find('.avatar-view').removeClass("required");
				$.CurrentNavtab.find('#salesEditViewForm').validator("setField", "image", "");
			}
			else
			{
				$.CurrentNavtab.find('#cid_welcometitle').css("display","");
				$.CurrentNavtab.find('#cid_image').css("display","");
				$.CurrentNavtab.find('#welcometitle').addClass("required");
				$.CurrentNavtab.find('#salesEditViewForm').validator("setField", "welcometitle", "required;");
				
				$.CurrentNavtab.find('#image_crop_avatar').find('.avatar-view').addClass("required");
				$.CurrentNavtab.find('#salesEditViewForm').validator("setField", "image", "required;");
			} 
		} 
}
$(function () {
	$.CurrentNavtab.find('.wxtype').on('ifChanged',function(e){
		wxtype_onchange();
    });  
	
	setTimeout("wxtype_onchange();",100); 
})
...
function addDiyDom(treeId, treeNode) {
	...
	{
		span += "【链接地址："; 
		span += "<span style=\"overflow: hidden; max-width: 700px;\">"+treeNode.key+"</span>";
		span += "】</font>";
	}
	else
	{
...
function wxmenusaddCatalog(obj,cp,label){
	var ajaxurl = 'index.php?module=Supplier_WxSettings&action=WxMenuItem&parentid='+cp;	
	if(cp == 'root')
		ajaxurl += '&root=true';
	else if(cp!='new')
		ajaxurl += '&node='+cp;
	$(this).dialog({id:"catalog",url:ajaxurl,title:"新建",height:300,width:800,mask:true,resizable:false,drawable:false,maxable:false});
}

function wxmenuseditCatalog(obj,cp,label){
	var ajaxurl = 'index.php?module=Supplier_WxSettings&action=WxMenuItem&record='+cp;	
	if(cp == 'root')
		ajaxurl += '&root=true';
	else if(cp!='new')
		ajaxurl += '&node='+cp;
	//jQuery.pdialog.open(ajaxurl,"catalog","编辑",{height:240,mask:true});
	$(this).dialog({id:"catalog",url:ajaxurl,title:"编辑",height:300,width:800,mask:true,resizable:false,drawable:false,maxable:false});
}

function wxmenusdeleteCatalog(obj,cp,label){
   var ajaxurl = 'index.php?module=Supplier_WxSettings&action=WxMenuItem&type=deleted&record='+cp;
	$(this).alertmsg('confirm',"您确定要删除当前菜单项？",{title: '删除菜单',mask:false,okName: "确定", cancelName: "取消",okCall: function(){
