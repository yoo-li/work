...
if (check_authorize('tezanadmin') || is_admin($current_user)){
    $searchpanel["供应商"]='<div style="display:block;float:left;width:200px;">
		<span class="ph-label" onclick="$(this).css(\'display\',\'none\');$(\'#suppliers_search_input\').focus();" style="display:inline;'.($_REQUEST["suppliers_search_input"] != ""?"display:none;":"").'">输入供应商名称</span>
		<input id="suppliers_search_input" class="search-text textInput" type="text" onblur="if (this.value == \'\') $(this).prev().css(\'display\',\'inline\');" onfocus="$(this).prev().css(\'display\',\'none\');" name="suppliers_search_input" value="'.$_REQUEST["suppliers_search_input"].'" style="width:180px;">
		</div>';
}
...
{ 
        $input=$_REQUEST["products"];
        $products_no=XN_Filter("my.mall_products_no","=",$input);
        $products_name=XN_Filter("my.productname","like",$input);
		global  $supplierid;
        $products=XN_Query::create("Content")
            ->tag("mall_products")
            ->filter("type","eic","mall_products")
            ->filter(XN_Filter::any($products_no,$products_name))
            ->filter("my.deleted","=","0")
        ...
        if(count($products) > 0)
		{
            ...
            foreach($products as $product_info)
			{
      	  	...
      	  	$query->filter("my.products","in",$product_ids);
		}
