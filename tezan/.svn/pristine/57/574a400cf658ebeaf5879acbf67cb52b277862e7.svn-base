...
$issupplier=XN_Query::create("Content_Count")
    ->tag("suppliers")
    ...
    ->rollup();
$issupplier->execute();
$count=$issupplier->getTotalCount();
if ($count>0){
    $pre_url="index.php?module=Logistics&action=supplierLogisticInfo";
    $changeArray['hideFields']='oper';
    $changeArray['operates']=array(
        "header"=>array('label'=>'配置物流信息','sort'=>false,'width'=>12,'align'=>'center'),
        "orientation"=>"vertical",
        "operate_link"=>array(
            ""=>array("url"=>$pre_url,"target"=>"navTab","label"=>"配置物流信息","rel"=>'edit')
            //""=>array("=",array("配置物流信息"=>$pre_url),"destination"=>array("target"=>"navTab","rel"=>"edit" )),
        )
    );
}
