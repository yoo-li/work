...
$content = '<table width="100%" cellspacing="0" border="0" align="center" style="font-family: Arial; font-size: 10pt">
    <tbody>
        ...
        <tr>
            <td valign="top" align="center" colspan="4" rowspan="1">
                <strong><span style="font-family: 黑体;font-size: 20px;">物流揽件对账单</span></strong>
            </td>
        </tr>		
        <tr>          
...
if (isset($_REQUEST['logisticpackageid']) && $_REQUEST['logisticpackageid'] != "")
{
    ...
    $templates['$logisticpackagename'] = $logisticpackage_info->my->serialname;
	...
	if (count($mall_logisticbills) > 0)
	{
		...
		{
			$orderid = $mall_logisticbill_info->my->orderid;
			$orderids[] = $orderid;
		}
	} 
	$orders = array();
...
{
		window.location="/index.php?module='.$currentModule.'&action=PrintLogisticStatistics&oper="+mode+"&logisticpackageid='.$logisticpackageid.'";
...
function print_out()
{
	window.open("/index.php?module='.$currentModule.'&action=PrintLogisticStatistics&oper=print&logisticpackageid='.$logisticpackageid.'");
