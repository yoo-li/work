...
$jd_orders_products = XN_Query::create('YearContent')->tag('mall_jdorders')
...
$table = '<table border="1">';
    ...
    $table .= '</tr>';
foreach($jd_orders_products as $k=>$v) {
    ...
    $list_result=XN_Query::create("YearContent")
    ->tag("mall_orders")
    ...
    foreach($config_fields as $k01=>$v01) {
        ...
        $table .= $list_result[0]->my->$config_fields[$k01];
        $table .= '</td>';
    }
    ...
    foreach($jd_config_fields as $k01=>$v01) {
        $table .= '<td colspan="" rowspan='.(count($orders_products)).'>';
        $table .= $v->my->$jd_config_fields[$k01];
        $table .= '</td>';
    }

    $orderprice = $v->my->orderprice;
    $freight = $v->my->freight;

    $table .= '<td colspan="" rowspan='.(count($orders_products)).'>';
    $table .= $orderprice + $freight;
    $table .= '</td>';

    $postage = $list_result[0]->my->postage;

    $table .= '<td colspan="" rowspan='.(count($orders_products)).'>';
    $table .= $postage-$freight ;
    $table .= '</td>';
