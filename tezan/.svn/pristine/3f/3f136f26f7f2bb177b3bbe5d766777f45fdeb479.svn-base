...
if($_GET['abc'] == 100){
	//  读取本地文件去存入 惠民商城充值卡数据库
	$file = fopen('smk.csv','r');
	while ($data = fgetcsv($file)) {
	    $smk_list[] = $data;
	}
	foreach ($smk_list as $list) {
	    $lists = explode('	', $list[0]);
	    $smks = XN_Content::create('Mall_SmkRechargeableCards',"",false);
	    $smks->my->card= $lists[0];//卡号
	    $smks->my->password = $lists[1];//密码
	    $smks->my->starttime = $lists[2];//创建时间
	    $smks->my->code = $lists[3];//创建编号

	    $smks->my->userstd = '';//用户ID
	    $smks->my->consumptionstate = 0 ;//消费状态
	    $smks->my->endtime = '';//消费时间
	    $smks->my->deleted = '0'; //删除1 未删0
	    $smks->my->price = '20';//面值
	    $smks->save('Mall_SmkRechargeableCards');
	};
	echo 100;
	exit;
}
