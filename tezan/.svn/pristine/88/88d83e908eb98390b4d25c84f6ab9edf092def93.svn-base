...
class Mall_SmkUsers extends CRMEntity {
	
	public $table_name = 'mall_smkusers';
	public $table_index= 'id';
	public $tab_name = Array('mall_smkusers');
	public $tab_name_index = Array('mall_smkusers'=>'id');
	public $customFieldTable = Array('mall_smkusers', 'id');
    ...
    function Mall_SmkUsers() {
		$this->column_fields = getColumnFields('Mall_SmkUsers');
	}
    ...
    public function approval_event($record,$event) {
        ...
        $lowermodule="mall_smkusers";
        if($event=="Agree"){
            //审批通过时更改审批状态，同时把不为空的内容更新到Suppliers表中，电话号码、邮箱、地址更新到profile表中
            $correctContent=XN_Content::load($record,"mall_smkusers");
