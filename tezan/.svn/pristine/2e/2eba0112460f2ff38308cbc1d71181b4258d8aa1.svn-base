...
+function ($) {
    ...
    var BJUI = {
        ...
        initLayout: function(ww) {
            ...
            $('#bjui-navtab .tabsPageContent').outerHeight(iContentH - navtabH)
    ...
    $(function() {
        ...
        $.validator && $.validator.config({
            ...
            rules: {
                ...
                ,date:[/^\d{4}[\/\-](0?[1-9]|1[012])[\/\-](0?[1-9]|[12][0-9]|3[01])$/, '请输入正确的日期']
                //,time: [/^([01]\d|2[0-3])(:[0-5]\d){1,2}$/, '请输入正确的时间,例:14:30或14:30:00']
                ,time: [/^(2[0123]|(1|0?)[0-9]){1}:([0-5][0-9]){1}:([0-5][0-9]){1}$/, '请输入正确的时间']
                ,datetime: [/^\d{4}[\/\-](0?[1-9]|1[012])[\/\-](0?[1-9]|[12][0-9]|3[01])\s+(2[0123]|(1|0?)[0-9]){1}:([0-5][0-9]){1}:([0-5][0-9]){1}$/,
                            '请输入正确的日期时间']
                ,simpledatetime: [/^\d{4}[\/\-](0?[1-9]|1[012])[\/\-](0?[1-9]|[12][0-9]|3[01])\s+(2[0123]|(1|0?)[0-9]){1}:([0-5][0-9]){1}$/,
                            '请输入正确的日期时间']
                ,ID_card: [/^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])((\d{4})|\d{3}[A-Z])$/, '请输入正确的身份证号码']
                ,url: [/^(https?|ftp):\/\/[^\s]+$/i, '网址格式不正确']
                ,postcode: [/^[1-9]\d{5}$/, '邮政编码格式不正确']
                ,chinese: [/^[\u0391-\uFFE5]+$/, '请输入中文']
                ,username: [/^\w{2,12}$/, '请输入2-12位数字、字母、下划线']
                ,ch_username: [/^[A-Za-z0-9\u0391-\uFFE5]{2,12}$/,'请输入2-12位数字、字母、中文、下划线']
                ,password: [/^[0-9a-zA-Z]{6,16}$/, '密码由6-16位数字、字母组成']
				,fieldname: [/^[a-zA-Z][0-9a-zA-Z\_]{4,}$/, "请输入大于等于4位字母开头、数字或下划线"]
...
+function ($) {
    ...
    window.FRAG = {
        ...
        maskProgress: '<div class="bjui-maskProgress bjui-ajax-mask"><i class="fa fa-cog fa-spin"></i>&nbsp;&nbsp;#progressmsg#<div class="progressBg"><div class="progress" style="width:10%;"></div></div></div>',
        ...
        gridPaging: '<ul class="pagination">' +
                    ...
                    '    </li>' +
                    '    <li class="page-jumpto"><span class="page-input" style="padding:4px 12px;"><input class="form-control input-sm-pages" type="text" size="3.2" value="#pageCurrent#" title="#jumpto#"></span></li>' +
...
+function ($) {
    ...
    $.fn.extend({
                ...
                success  : function(response) {
                    ...
                    if (!json[BJUI.keys.statusCode]) {
						try{
							...
							$this.parent().append($ajaxMask)
                            $this.append($ajaxMask)
							...
							if (op.loadingmask) {								
								...
								{
									$ajaxMask.find('.progress').animate({width:'100%'}, 'fast', function() {
									...
									$.each($ajaxMask,function(i){$ajaxMask[i].remove()})
								}
							}
						}catch(err){
							$ajaxMask.fadeOut('normal', function() { $(this).remove() })
							$this.alertmsg('error',err.message,{title:err.name})
						}
                    } else {
        ...
        resizePageH: function() {
            return this.each(function() {
				...
				if ($pageContent.find('.bjui-pageContent').length > 0){
					$pageContent.find('.bjui-pageContent').each(function(e){
						var mr = parseInt($(this).css("marginLeft"),10)+parseInt($(this).css("marginRight"),10);
						var pt = $(this).parent().width();
						if (pt > 0 && mr > 0) {
							$(this).width(pt-mr);
						}
					});
               ...
               setTimeout(function() {$pageContent.trigger(BJUI.eventType.resizeGrid)}, 30)
            })
        },
    ...
    $.extend(Array.prototype, {
        remove: function(index) {
        	if (isNaN(index))
			{
				this.splice(this.indexOf(index) == -1 ? this.length : this.indexOf(index), 1);
			}else{
...
+function ($) {
    ...
    Slidebar.prototype.lock = function() {
        ...
        that.$split.show()  
		//oldhand modify 处理编辑视图 不会随窗口变化的问题 
    ...
    Slidebar.prototype.initHnav = function() {
        var that   = this,
            ...
            active = $box.data("active") ? $box.data("active") : 0,
        ...
        $array.each(function(i) {
            ...
            $box.append($panel)
            if (i != active) $panel.collapse('show')
        })
...
+function ($) {
    ...
    Navtab.prototype.reloadFlag = function(tabids, isSwitch) {
        ...
        for (var i = 0; i < arr.length; i++) {
            ...
            if ($tab) {
                if (this.tools.indexTabId(arr[i]) == currentIndex){
                    ...
                    if(i == 0 && isSwitch){
                        var index = this.tools.indexTabId(arr[i])
...
+function ($) {
    ...
    Bjuiajax.prototype.ajaxcallback = function(json) {
        ...
        if (json[BJUI.keys.statusCode] == BJUI.statusCode.ok) {
            ...
            if (tabids.length) {
                $.unique(tabids)
                var isSwitch = !json.forward || false

                setTimeout(function() { BJUI.navtab('reloadFlag', tabids.join(','),isSwitch) }, 100)
            }
            if (dialogids.length) {
                $.unique(dialogids)
    ...
    Bjuiajax.prototype.dialogcallback = function(json) {
		...
		if (json.reload && !forward)
		{
			setTimeout(function() { BJUI.navtab('reload', {url:json.reload}) }, 100);
		}
    }
    ...
    Bjuiajax.prototype.doajax = function(options) {
        var that = this, $target, $element = that.$element,idname
    ...
    Bjuiajax.prototype.ajaxform = function(option) {
		...
		if (formcallback != "" &&  formcallback != undefined)
		{	
			try
			{
				var fn = window[formcallback];
				if (!fn($form))
				{
					return false;
				}			 
    ...
    Bjuiajax.prototype.doajaxchecked = function(options) {
        ...
        var todo = function() {
			...
			if (options.dialog) {
				...
				$element.doAjax({type:options.type, url:options.url, data:options.data, loadingmask:options.loadingmask?options.loadingmask:false, callback:callback ? callback : $.proxy(function(data) {that.ajaxcallback(data)}, that)})
			}
        }
	...
	/*
    $(document).on('click.bjui.bjuiajax.data-api', '[data-toggle="dodialogchecked"]', function(e) {
        var $this = $(this), options = $this.data()
		...
		Plugin.call($this, 'dodialogchecked', options)
            ...
            open: function(type, msg, buttons) {
                ...
                if (buttons) {
                    for (var i = 0; i < buttons.length; i++) {
                        ...
                        if(buttons[i].name!==false && buttons[i].name!="false"){
                ...
                if (that.options.autoClose || that.options.types.info == type || that.options.types.correct == type) {
                    if (alertTimeout) {
    ...
    Pagination.prototype.init = function() {
            ...
            $button.on('click', function() {
                ...
                if (pageCurrent && pageCurrent.isPositiveInteger()) {
                    ...
                    if(callback!="" && typeof callback ==="function"){
                        callback(pageCurrent, options.pageSize);
                    }
                    else {
        ...
        function _bindEvent($target, pageCurrent) {
            $target.on('click', function(e) {
                ...
                if(callback!="" && typeof callback ==="function"){
                    callback(pageCurrent, that.options.pageSize);
                }
                else
                {
                    $(this).bjuiajax('pageCallback', pagingInfo, that.$element.closest('.bjui-layout'))
...
+function ($) {
	...
	// oldhand modify 为了处理日期，"yyyy-MM-dd" 与 "yyyyMMdd" 同时兼容
    $(document).on('blur.bjui.datepicker.data-api', '[data-toggle="datepicker"]', function(e) {
        var $this = $(this)  
		...
		if (pattern == "yyyy-MM-dd")
		{
			...
			if(!regexp.test(datepicker_value))   
			{
				...
				if(newregexp.test(datepicker_value))  
				{
					...
					var that = $this;
					setTimeout(function() { that.val(newvalue); },10); 
				}  
				else
				{
					...
					{
						var that = $this;
						setTimeout(function() { that.val(""); },10); 
					}  
				} 
			}    
		} 
    ...
    $(document).on('click.bjui.datepicker.data-api', '[data-toggle="datepicker"]', function(e) {
        var $this = $(this) 
        // oldhand modify 为了处理日期，只有点击按钮才弹出日期选择框
        if ($this.data('onlybtn') || $this.data('onlybtn') == undefined ) return
        ...
        var tools = {
            ...
            jumpPage: function(pageCurrent, pageSize) {
                ...
                if (options.local === 'remote') {
                    if (callback && typeof callback === 'function') {
                        callback( options.pageCurrent, pageSize)
                    } else {
    ...
    Tablefixed.prototype.initBody = function() {
        ...
        if (this.options.height) {
            ...
            var resizeH = function() {
				...
				var foot_height = 0;
				if (that.$fixed.parent().find('.bjui-pageFooter'))
				{
					foot_height = that.$fixed.parent().find('.bjui-pageFooter').height() + 3; 
				} 
                that.$fixed.parent().css('overflow', 'hidden')
                that.$fixed.height(_height)
                    .find('.fixedtableScroller').height(_height - that.$fixed.find('.fixedtableHeader').height() - foot_height)
            }
                        ...
                        $fixed.find('> .resizeProxy')
                            ...
                            .basedrag({
                                ...
                                stop:  function() {
									...
									$fixed.find('.table').css("max-width",tableW + move);
            ...
            $('div.bjui-resizeGrid').each(function() {
                ...
                if (width) {
                    $this.find('.bjui-tablefixed').each(function() {
						...
						$fixed.find('.table').css("max-width",realWidth); 
                ...
                $fixed.height(height)
                    .find('.fixedtableScroller').height(height - fixedH - foot_height)
...
+function ($) {
    ...
    Lookup.prototype.init = function() {
        ...
        if (!options.url) {
            ...
            options.url = encodeURI(options.url)
            if (options.checkgroup) {
            	var ids      = [],
                $checks  = $.CurrentNavtab.find(':checkbox[name='+ options.checkgroup +']:checked');
	            if (!$checks.length) {
	                BJUI.alertmsg('error', BJUI.regional.notchecked)
	                return
	            }
				...
				if (tmpUrl.indexOf(tmpPt+options.checkgroup+"=") >= 0) {
					var subUrl = tmpUrl.substring(0,tmpUrl.indexOf(tmpPt+options.checkgroup+"="));
						tmpUrl = subUrl + tmpPt + options.checkgroup+"="+ids.join(',');
				}else{
					tmpUrl += tmpPt+options.checkgroup+"="+ids.join(',');
				}
				options.url = tmpUrl;
            }
        }
    ...
    Lookup.prototype.LookupElement = function() {
    ...
    Lookup.prototype.setVal = function(args, type) {
		...
		var callback = $currentLookup.attr("data-callback");
		var group = $currentLookup.attr("data-group");
    ...
    function Plugin(option) {
        ...
        return this.each(function () {
            ...
            if(data.options.title != $this.data('title') ){
                data.options.title = $this.data('title');
            }
            if (typeof property === 'string' && $.isFunction(data[property])) {
                [].shift.apply(args)
                if (!args) data[property]()
...
+function ($) {
    ...
    Theme.prototype.init = function() {
		...
		$("#bjui-themes").find('a').on('click', function() {
			var themeName = $(this).attr('data-theme');
			if ($.cookie) $.cookie('bjui_theme', themeName, { path: '/', expires: 30 }); 
		})
...
+function ($) {
    ...
    $(document).on(BJUI.eventType.beforeInitUI, function(e) {
        ...
        //progress
		if ($box.find('> .bjui-maskProgress').find('.progress')) {
			$box.find('> .bjui-maskProgress').find('.progress').stop().animate({width:'75%'}, 'fast')
		}
...
+function ($) {
    ...
    $(document).on(BJUI.eventType.initUI, function(e) {
        ...
        /* TreeGrid */
        if ($.fn.treegrid) {
            var $treegrid = $box.find('table[data-toggle="treegrid"]');
			$treegrid.each(function(i) {
        ...
        if ($.fn.zTree) {
            ...
            $selectzTree.each(function() {
                ...
                var setPosition = function($box) {
                    ...
                    if (dialog && dialog.length) {
                        var treeMaxHeight = $(window).height() - top - h - 10,
                            treeOffset = $(window).height() / 2 - top,
                            treeTop = top + h;
                        if(treeOffset < 0){
                            treeTop = 10;
                            treeMaxHeight = $(window).height() - ($(window).height()-top) - treeTop
                        }
                        $box.css({top: treeTop, left: left, 'max-height': treeMaxHeight})
                    }else
                    {
		...
		/*buidu editor*/
		if (typeof UE === "object") {
			$box.find('[data-toggle="ueditor"]').each(function() {
				var $editor = $(this), options = $editor.data();
				var editName = $editor.attr("id");
				var maximumWords = options.maxlength || 20000;
				var readonly = options.readonly || false;
				if (editName == '') {
					editName = 'ueditor'
				}
				
				UE.delEditor(editName);
				UE.getEditor(editName,{
        ...
        $box.find('[data-toggle="dropdown"]').parent().on('show.bs.dropdown', function(e) {
			...
			var btn = $menu.prev();
			if (btn.hasClass("btn") > 0)
			{   
				var left = $menu.prev().offset().left - $menu.parent().offset().left; 
				var top = $menu.prev().height()+7; 
				$menu.css('left',  left); 
				$menu.css('top',  top); 
			}
			else
        ...
        * @author   oldhand
        * ======================================================================== */
       var $poshytips = $box.find('[data-toggle="poshytip"]');
       
       $poshytips.each(function(){
           var $element = $(this);
		   var elementtitle = $element.attr('title');
		   if (typeof elementtitle !== 'undefined') 
		   {
		   		$element.poshytip({allowTipHover:true});
		   }
		   else
		   {
			   if ($element.is('input'))
			   {
		  		 	elementtitle = $element.val();
					if (elementtitle != "")
					{
		 ...
		 * @author   徐雁
		 * ======================================================================== */
		$poshytips = $box.find('[data-hint="poshytip"]');
		$poshytips.each(function(){
			var $element = $(this);
			var elementtitle = $element.attr('title');
			if (typeof elementtitle !== 'undefined')
