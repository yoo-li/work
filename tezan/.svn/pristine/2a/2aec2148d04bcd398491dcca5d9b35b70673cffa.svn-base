                    ...
                    <td  colspan="3" align="left">
                        <span style="position: relative; display: inline-block;width:200px;" class="wrap_bjui_btn_box"> 
                            ...
                            <input readonly type="text" class="form-control"  style="padding-right: 25px; cursor: pointer; width: 200px;" size="20" onclick="$(this).parent().find(\'a.bjui-lookup\').trigger(\'click\');" value="" id="products_name" name="products_name">
                             <a  class="bjui-lookup" href="index.php?module=Mall_Products&action=massPopup&Popuptype=Mall_RobSingles&callback=Mall_RobSingles_details_massPopupcallback&filter='.$filter.'&selected_products=" onclick="changehref(this);" data-title="选择促销商品" data-toggle="lookupbtn" data-height="500" data-width="800" style="height: 22px; line-height: 22px;">
    ...
    $html.='<tr>
            ...
            <thead>
                <tr>
                    <th align="center" width="8%">编号</th><th align="center" width="40%">商品选择</th><th align="center" width="20%">秒杀价格</th><th align="center" width="20%">数量</th>';
        if($readonly!='true'){
        ...
        if(count($salesactivitys_products) > 0)
        {
            ...
            foreach($salesactivitys_products as  $salesactivitys_product_info)
            {
                    ...
                    if($readonly!='true'){
//                        $html.='<td align="center"><input placeholder=" " class="form-control required number"   data-rule="required;number; " name="num['.$index.']" id="num'.$index.'" value="'.$disaccount_num.'"></td>';
                        $html.='<td align="center"><input placeholder=" " class="form-control required number"   data-rule="required;number; " name="num['.$index.']" id="num'.$index.'" value="'.$salesactivitys_product_info->my->robprice.'"></td>';
                    }else{
                        $html.='<td align="center"><input name="num['.$index.']" class="form-control required number" readonly id="num'.$index.'" value="'.$salesactivitys_product_info->my->robprice.'"></td>';
                    }
                    $html.='<td align="center"><input name="activitynumber['.$index.']" class="form-control required number" readonly id="num'.$index.'" value="'.$salesactivitys_product_info->my->activitynumber.'"></td>';
