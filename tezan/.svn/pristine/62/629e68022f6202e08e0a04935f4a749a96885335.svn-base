 ...
 */
try{
    if($_REQUEST['profileid'] && !isset(XN_Profile::$VIEWER)){
        XN_Profile::$VIEWER=$_REQUEST['profileid'];
    }
    if($_REQUEST['currentModule'] && !isset($currentModule)){
        $currentModule=$_REQUEST['currentModule'];
    }
    if($_REQUEST['supplierid'] && !isset($supplierid)){
    ...
    $ma_saleorders=$loadContent->my->ma_saleorders;
    ...
    $pickorders = XN_Query::create("Content")
        ...
        ->filter("my.ma_inventorysaleout", "=", $record)
    ...
    $pickdetails=XN_Query::create("Content")
        ...
        ->end(-1)
        ->execute();
    $out_product_ids=array();
    foreach($pickdetails as $detail_info){
        $out_product_ids[]=$detail_info->my->ma_products;
    }
    $out_product_number=count($out_product_ids);
    try{
        $ingout_product_ids=XN_MemCache::get(XN_Application::$CURRENT_URL.$ma_storagelistfrom);
        if($out_product_number<count($ingout_product_ids)){
            foreach($out_product_ids as $ma_products){
                if(in_array($ma_products,$ingout_product_ids)){
                    echo '{"statusCode":"300","message":"产品正在出入库操作,请等待5秒钟之后再来操作"}';
                    die();
                }
            }
        }
        else{
            foreach($ingout_product_ids as $ma_products){
                if(in_array($ma_products,$out_product_ids)){
                    echo '{"statusCode":"300","message":"产品正在出入库操作,请等待5秒钟之后再来操作"}';
                    die();
                }
            }
        ...
        try{
            $ingin_product_ids=XN_MemCache::get(XN_Application::$CURRENT_URL.$ma_storagelistto);
            if($out_product_number<count($ingin_product_ids)){
                foreach($out_product_ids as $ma_products){
                    if(in_array($ma_products,$ingin_product_ids)){
                        echo '{"statusCode":"300","message":"产品正在出入库操作,请等待5秒钟之后再来操作"}';
                        die();
                    }
                }
                ...
                foreach($ingin_product_ids as $ma_products){
                    if(in_array($ma_products,$out_product_ids)){
                        echo '{"statusCode":"300","message":"产品正在出入库操作,请等待5秒钟之后再来操作"}';
                        die();
                    }
            ...
            $new_ingout_product_ids=array_merge($ingout_product_ids,$out_product_ids);
            $new_ingin_product_ids=array_merge($ingin_product_ids,$out_product_ids);
            XN_MemCache::put($new_ingout_product_ids,XN_Application::$CURRENT_URL.$ma_storagelistfrom);
            XN_MemCache::put($new_ingin_product_ids,XN_Application::$CURRENT_URL.$ma_storagelistto);
        }
        catch(XN_Exception $e){
            $new_ingout_product_ids=array_merge($ingout_product_ids,$out_product_ids);
            XN_MemCache::put($new_ingout_product_ids,XN_Application::$CURRENT_URL.$ma_storagelistfrom);
            XN_MemCache::put($out_product_ids,XN_Application::$CURRENT_URL.$ma_storagelistto);
        }
    }
    catch(XN_Exception $e){
        try{
            ...
            $new_ingin_product_ids=array_merge($ingin_product_ids,$out_product_ids);
            XN_MemCache::put($new_ingin_product_ids,XN_Application::$CURRENT_URL.$ma_storagelistto);
            XN_MemCache::put($out_product_ids,XN_Application::$CURRENT_URL.$ma_storagelistfrom);
        }
        catch(XN_Exception $e){
            XN_MemCache::put($out_product_ids,XN_Application::$CURRENT_URL.$ma_storagelistfrom);
            XN_MemCache::put($out_product_ids,XN_Application::$CURRENT_URL.$ma_storagelistto);
    ...
    foreach($pickorders as $pickorder_info){
        ...
        if(count($query)){
            ...
            $recheck_order->my->supplierid=$loadContent->my->submit_id;
            $recheck_order->my->number+=$pickorder_info->my->submitnumber;
            $recheck_order->my->recheckorders_status="1";
            $recheck_order->save("ma_recheckorders");
            $recheckordersid=$recheck_order->id;
        }
        else
        {
            $recheck_order=XN_Content::create("ma_recheckorders","",false);
            $recheck_order->my->send_supplierid=$loadContent->my->send_supplierid;
            $recheck_order->my->supplierid=$loadContent->my->submit_id;
            $recheck_order->my->ma_purchaseorders=$loadContent->my->ma_purchaseorders;
            $recheck_order->my->ma_saleorders=$loadContent->my->ma_saleorders;
            ...
            $recheck_order->my->recheckorders_status="1";
            $recheck_order->my->submit_user=XN_Profile::$VIEWER;
    ...
    XN_Content::batchsave($pickorders,"ma_pickorders");
    ...
    $certificateout->my->record= $record;
    $certificateout->my->ma_pickorders= $ma_pickorders;
    $certificateout->my->ma_recheckorders= $recheckordersid;
    $certificateout->my->ma_purchaseorders=$loadContent->my->ma_purchaseorders;
    $certificateout->my->ma_saleorders=$loadContent->my->ma_saleorders;
    $certificateout->my->ma_inventorysaleout=$record;
    $certificateout->my->certificate_type="saleout";
    $certificateout->my->submit_type=$loadContent->my->submit_type;
    $certificateout->my->submit_id=$loadContent->my->submit_id;
    $certificateout->my->receipt_type=$loadContent->my->receipt_type;
    $certificateout->my->receipt_id=$loadContent->my->receipt_id;
    ...
    $saleout_details=XN_Query::create("Content")
        ->tag("ma_saleoutdetails")
        ->filter("type","eic","ma_saleoutdetails")
        ->filter("my.record","=",$record)
        ->filter("my.deleted","=","0")
        ->end(-1)
        ->execute();
    $factorys_names=array();
    foreach($saleout_details as $saleout_detail){
        $saleout_detail->my->submitnumber+=$product_submitnumbers[$saleout_detail->my->ma_products];
        $factorys_names[$saleout_detail->my->factorys]=$saleout_detail->my->factorys_name;
    }
    XN_Content::batchsave($saleout_details,"ma_saleoutdetails");
    ...
    foreach(array_chunk($product_ids,50) as $chunk_product_ids) {
        $from_inventorycounts = XN_Query::create("Content")
            ...
            ->filter("my.supplierid", "=", $submit_id)
            ->filter("my.deleted", "=", "0")
            ->end(-1)
            ->execute();
    ...
    foreach($product_submitnumbers as $ma_products=>$number){
        ...
        else
        {
            ...
            $inventorycount_info->my->supplierid=$submit_id;
    ...
    foreach($pickdetails as $detail_info){
        ...
        foreach($batch_infos as $index=>$number_arr){
            ...
            else
            {
                ...
                $frominventory_info->my->supplierid=$submit_id;
            ...
            $billwaterContent->my->ma_purchaseorders=$ma_purchaseorders;
            $billwaterContent->my->ma_saleorders=$ma_saleorders;
            ...
            $billwaterContent->my->receipt_type=$receipt_type;
            $billwaterContent->my->submit_type=$submit_type;
            $billwaterContent->my->submit_id=$submit_id;
            $billwaterContent->my->receipt_id=$receipt_id;
            $billwaterContent->my->execute=XN_Profile::$VIEWER;
            ...
            else
            {
                ...
                $toinventory_info->my->supplierid=$submit_id;
        ...
        if($wcs_order_info->my->wcs_ordersstatus!="拣货完成"){
            if($wcs_order_info->my->wcs_ordersstatus!="正在拣货"){
                ...
                foreach($pickdetails as $pickdetail_info)
                {
                    ...
                    $barcode = $pickdetail_info->my->barcode;
                    ...
                    if (count($wcs_warehouselocationsproducts) > 0)
                    {
                        ...
                        $product_barcode = $wcs_warehouselocationsproduct_info->my->barcode;
                ...
                XN_Content::batchsave($wcs_orderdetails,"wcs_orders_details");
    ...
    $ingout_product_ids=XN_MemCache::get(XN_Application::$CURRENT_URL.$ma_storagelistfrom);
    $ingin_product_ids=XN_MemCache::get(XN_Application::$CURRENT_URL.$ma_storagelistto);
    $new_ingin_product_ids=array_diff($ingin_product_ids,$out_product_ids);
    $new_ingout_product_ids=array_diff($ingout_product_ids,$out_product_ids);
    XN_MemCache::put($new_ingout_product_ids,XN_Application::$CURRENT_URL.$ma_storagelistfrom);
    XN_MemCache::put($new_ingin_product_ids,XN_Application::$CURRENT_URL.$ma_storagelistto);
    echo '{"status":"1","statusCode":200,"message":null,"tabid":"edit","closeCurrent":true,"forward":null}';
}
catch(XN_Exception $e){
    echo '{"status":"0","statusCode":300,"message":"'.$e->getMessage().'"}';
}
die();
