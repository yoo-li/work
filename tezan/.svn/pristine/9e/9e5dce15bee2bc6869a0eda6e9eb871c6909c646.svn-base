...
try {
	...
	$record = $focus->id;
	$loadcontent = XN_Content::load($record,"domains");
	$appname = $loadcontent->my->domain;
	$domaindescription = $loadcontent->my->domaindescription;
	$app = XN_Application::load($appname);
	$app->description = $domaindescription;
	$app->trialtime = $loadcontent->my->trialtime;
	$app->save($appname);
