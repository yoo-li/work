...
$query = XN_Query::create ( 'Content' ) ->tag($focus->table_name)
	->filter ( 'type', 'eic', $focus->table_name)
	->filter ( 'my.deleted', '=', '0' );	
$statistics = XN_Query::create ( 'Content_Count' )->tag($focus->table_name)
    ->filter ( 'type', 'eic', $focus->table_name)
    ->filter ( 'my.deleted', '=', '0' )
    ->begin(0)->end(-1)
    ->rollup('my.amount');
...
/*
$statistics = $statistics->execute();
if(count($statistics)>0)
    $smarty->assign("CUSTOMSHOWMSG", "<font color=blue>&nbsp;&nbsp;&nbsp;&nbsp;金額总合计：".$statistics[0]->my->amount . "</font>");
*/
