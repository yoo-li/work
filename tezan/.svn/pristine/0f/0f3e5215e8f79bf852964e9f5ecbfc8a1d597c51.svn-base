...
if(is_admin($current_user)||check_authorize('tezanadmin')) {
    $searchpanel['商家名称'] = '
			<span>
				<input name="' . $currentModule . '_suppliername" placeholder="请输入商家名称" value="' . $_REQUEST[$currentModule . "_suppliername"] . '" style="width:180px;">
			</span>';
}
$searchpanel['员工']='
        <span>
            <input name="'.$currentModule.'_profileid" placeholder="请输入员工名称" value="'.$_REQUEST[$currentModule."_profileid"].'" style="width:180px;">
        </span>';
$searchpanel['订单号']='
        <span>
            <input name="'.$currentModule.'_orders_no" placeholder="请输入订单号" value="'.$_REQUEST[$currentModule."_orders_no"].'" style="width:180px;">
        </span>';
...
$query = XN_Query::create ( 'Content' ) ->tag($focus->table_name)
	->filter ( 'type', 'eic', $focus->table_name)
	->filter ( 'my.deleted', '=', '0' );
if(isset($_REQUEST[$currentModule.'_orders_no']) && $_REQUEST[$currentModule.'_orders_no'] != ''){
    $order_query=XN_Query::create("Content")
        ->tag("mall_orders")
        ->filter("type","eic","mall_orders")
        ->filter("my.mall_orders_no",'like',$_REQUEST[$currentModule.'_orders_no'])
        ->filter('my.deleted','=','0')
        ->end(5)
        ->execute();
    ...
    if(count($order_query)){
        $order_ids=array();
        foreach ($order_query as $order_info){
            $order_ids[]=$order_info->id;
        }
        $query->filter ( 'my.orderid', 'in', $order_ids);
    }
    else{
        $query->filter ( 'my.orderid', '=', '0');
    }
}
if(isset($_REQUEST[$currentModule.'_profileid']) && $_REQUEST[$currentModule.'_profileid'] != ''){
    $profile = XN_Query::create('Profile')
        ->tag("profile")
        ->filter('type', '=', 'pt')
        ->filter('givenname', 'like', $_REQUEST[$currentModule.'_profileid'])
        ->begin(0)->end(5)
        ->execute();

    if(count($profile)){
        $profile_ids=array();
        foreach ($supplier_query as $supplier_info){
            $profile_ids[] = $pinfo->screenName;
        }
        $query->filter ( 'my.profileid', 'in', $profile_ids);
    }
    else{
...
if(is_admin($current_user)||check_authorize('tezanadmin')){
    if(isset($_REQUEST[$currentModule.'_suppliername']) && $_REQUEST[$currentModule.'_suppliername'] != ''){
        $supplier_query=XN_Query::create("Content")
            ->tag("suppliers")
            ->filter("type","eic","suppliers")
            ->filter("my.suppliername",'like',$_REQUEST[$currentModule.'_suppliername'])
            ->filter('my.deleted','=','0')
            ->end(5)
            ->execute();

        if(count($supplier_query)){
            $supplier_ids=array();
            foreach ($supplier_query as $supplier_info){
