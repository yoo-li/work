...
$query = XN_Query::create ( 'Message' ) 
	->filter ( 'my.messagetype', '=', '1' )
	->filter ( 'my.deleted', '=', '0' );
...
if (count($supplier_users) > 0)
{
	...
	if (count($wxsettings) > 0)
	{
		...
		$query->filter ( 'my.appid', '=', $wxsetting_info->id);
	}
...
/*
$pre_url="index.php?module=LoginLogs&action=ListView&from=LastLoginLog";
$changeArray['operates']=array(
    "header"=>array('label'=>'查看历史','sort'=>false,'width'=>12,'align'=>'center'),
    "orientation"=>"vertical",
    "fieldbase"=>'',
    "operate_link"=>array(
        ""=>array(array("label"=>"查看历史","url"=>$pre_url,"target"=>"navTab","rel"=>"LoginLogs" )),
    )
);
*/
