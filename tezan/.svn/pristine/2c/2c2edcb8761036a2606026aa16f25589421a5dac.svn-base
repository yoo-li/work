<?php
	...
	function  re_initdata()
	{
		try
		{
			...
			XN_MemCache::put('{"statusCode":200,"progress":"2","message":"初始化企业管理用户"}',"get_longtime_status_Wait_Init_App");
			create_user();

			XN_MemCache::put('{"statusCode":200,"progress":"3","message":"初始化主菜单"}',"get_longtime_status_Wait_Init_App");
			create_parent_tabs();

			XN_MemCache::put('{"statusCode":200,"progress":"4","message":"初始化Eventhandlers"}',"get_longtime_status_Wait_Init_App");
			make_eventhandlers();

			XN_MemCache::put('{"statusCode":200,"progress":"5","message":"初始化ws_operations"}',"get_longtime_status_Wait_Init_App");
			make_ws_operations();

			XN_MemCache::put('{"statusCode":200,"progress":"6","message":"初始化ws_fieldtypes"}',"get_longtime_status_Wait_Init_App");
			...
			XN_MemCache::put('{"statusCode":200,"progress":"10","message":"初始化所有模块"}',"get_longtime_status_Wait_Init_App");
			init_all_module(10);

			XN_MemCache::put('{"statusCode":200,"progress":"60"}',"get_longtime_status_Wait_Init_App");
			...
			if (count($profiles) > 0)
			{
				...
				foreach($profiles as $profiles_info)
				{
					...
					XN_MemCache::put('{"statusCode":200,"progress":"'.(60+intval(35/count($profiles)*$index)).'","message":"初始化标准读写权限"}',"get_longtime_status_Wait_Init_App");
					$index++;
				}
			...
			{
				XN_MemCache::put('{"statusCode":200,"progress":"80","message":"初始化标准读写权限"}',"get_longtime_status_Wait_Init_App");
				initprofilebyname("标准读写权限");
			}

			init_select_program();
			XN_MemCache::put('{"statusCode":200,"progress":"95"}',"get_longtime_status_Wait_Init_App");
		...
		{
			XN_MemCache::put('{"statusCode":300,"message":"'.$e->getMessage().'","status":"complete"}',"get_longtime_status_Wait_Init_App","120");
			die();
		}
	}
	...
	function createroles() {
		...
		{
			throw new XN_Exception("初始化部门树失败!<br>");
		}
	}
	...
	function initadminprofile()
	{
		try
		{
			...
			throw new XN_Exception("初始化BOSS权限失败!<br>");
		}
	}
	...
	//第七步
	function init_all_module($progress)
	{
		...
		foreach ( $allmodules as $module)
		{
			...
			XN_MemCache::put('{"statusCode":200,"progress":"'.($progress+intval(50/count($allmodules)*$index)).'","message":"初始化模块：'.$module.'"}',"get_longtime_status_Wait_Init_App");
			$index++;
		}
		foreach ( $cf_fields as $cf_field_info)
		{
	...
	function modules_list()
	{
		...
		$configpath = $_SERVER['DOCUMENT_ROOT'].'/admin/initapp.module.config.php';
		...
		while (false !== ($file = readdir($dir)))
		{
			...
			{
				if (isset($initmodule) && isset($_REQUEST["m"]) && $_REQUEST["m"] != "")
				{
					$pl = strtolower($_REQUEST["m"]);
					if ((isset($initmodule[$pl]) && in_array($file, $initmodule[$pl])) || (isset($initmodule["all"]) && in_array($file, $initmodule["all"])))
					{
						$filelist[] = $file;
					}
				}
