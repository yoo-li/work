    ...
    try {
		...
		if (count($mall_settings) > 0)
		{
			...
			{ 
				...
				$logisticbill_info->my->orderid = $orderid;
				...
				$logisticbill_info->my->vendorid = $vendorid;  
				...
				$mall_settlementorders = XN_Query::create ( 'YearContent' ) ->tag('mall_settlementorders')
				    ...
				    ->filter ( 'my.deleted', '=', '0' )
				    ->filter ( 'my.orderid', '=' ,$orderid)
					->filter ( 'my.mall_settlementordersstatus', '=' ,'待发货')
					->end(-1)
				    ->execute(); 
				...
				if (count($mall_settlementorders) > 0)
				{
					$calendars = XN_Query::create ( 'Content' ) ->tag('calendar')
					    ...
					    ->filter ( 'my.deleted', '=', '0' )
					    ->filter ( 'my.personman', '=' ,XN_Profile::$VIEWER) 
				...
				try {
					...
					$smsCon = '您的订单'.$order_info->my->mall_orders_no.'已经发货,物流公司:'.$mylogisticname.',发货单号:'.$invoicenumber.',预计12小时后,物流信息将会更新.';
					global  $supplierid; 
