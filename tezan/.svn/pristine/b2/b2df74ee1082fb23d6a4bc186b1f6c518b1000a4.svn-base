...
+function ($) {
    ...
    function fixedLeft($msgbox) {
        ...
        if (($input  && $input.is(':hidden')) && ($btngroup && $btngroup.hasClass('bootstrap-select'))) {
        ...
        'red_right_effect': {
            ...
            msgShow: function($msgbox, type) {
                ...
                if ($wrap.hasClass('n-error'))
                    $icon.removeClass('n-icon').addClass('glyphicon glyphicon-remove-circle').css({ 'font-size':'14px', 'vertical-align':'middle'})
                else if ($wrap.hasClass('n-tip'))
                    $icon.removeClass('n-icon').addClass('glyphicon glyphicon-info-sign').css({ 'font-size':'14px', 'vertical-align':'middle'})
                else if ($wrap.hasClass('n-ok'))
                    $icon.removeClass('n-icon').addClass('glyphicon glyphicon-ok-circle').css({ 'font-size':'14px', 'vertical-align':'middle'})
                if ($el.is(':animated')) return
                if (type === 'error') {
                    $el.css({
                        left: offLeft + 20,
