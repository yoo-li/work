...
if ($_REQUEST['approvalstatus']  == "2" || $_REQUEST['approvalstatus']  == "3")
{
	$_SESSION['self_approvalstatus'] = "0";
}

if(isset($_SESSION['self_approvalstatus']) && $_SESSION['self_approvalstatus'] !='')
{
    $self_approvalstatus = $_SESSION['self_approvalstatus'];
}
else
{
	$self_approvalstatus = "1";
}
 
$searchpanel["我的审批状态"]='<a href="javascript:;" '.($self_approvalstatus=="0"?'class="over" ':'').' for="approval_self_approvalstatus_period" data-key="0" title="全部" style="margin:auto 1px;">全部</a>&nbsp;<a href="javascript:;"  '.($self_approvalstatus=="1"?'class="over" ':'').'  for="approval_self_approvalstatus_period" data-key="1" title="待审" style="margin:auto 1px;">待审</a>&nbsp;<a href="javascript:;"    '.($self_approvalstatus=="2"?'class="over" ':'').' for="approval_self_approvalstatus_period" data-key="2" title="已审" style="margin:auto 1px;">已审</a><input value="" type="hidden" id="search_approval_self_approvalstatus" name="self_approvalstatus" />
						<script type="text/javascript">
							$(document).ready(function()
							{
								$(\'a[for="approval_self_approvalstatus_period"]\').each(function(){
									$(this).click(approval_self_approvalstatus_period_onclick);
									});
							});
							function approval_self_approvalstatus_period_onclick() {
								var hasClass = true;
								if ($(this).hasClass("over")){
									hasClass = false;
								}
								$(\'a[for="approval_self_approvalstatus_period"]\').toggleClass("over",false);
								if (hasClass){
									$(this).addClass("over");
									$(\'#search_approval_self_approvalstatus\').val($(this).data("key"));
								}else{
									$(\'#search_approval_self_approvalstatus\').val("");
								}
							}
						</script>';
...
{
	if($_REQUEST['self_approvalstatus'] == "1")
	{
		 $_SESSION['self_approvalstatus'] = '1';
		 $query->filter("my.pendingapprover", '=', XN_Profile::$VIEWER);
	}
	else if($_REQUEST['self_approvalstatus'] == "2")
	{
		 $_SESSION['self_approvalstatus'] = '2';
		 $query->filter("my.approver", '=', XN_Profile::$VIEWER);
	}
	else if($_REQUEST['self_approvalstatus'] == "0")
	{
		$_SESSION['self_approvalstatus'] = '0';
 	    $approver = XN_Filter("my.approver", '=', XN_Profile::$VIEWER);
 	    $pendingapprover   = XN_Filter("my.pendingapprover", '=', XN_Profile::$VIEWER);
 	    $query->filter(XN_Filter::any($approver, $pendingapprover));
	}
	else
	{ 
	    $query->filter("my.pendingapprover", '=', XN_Profile::$VIEWER);
