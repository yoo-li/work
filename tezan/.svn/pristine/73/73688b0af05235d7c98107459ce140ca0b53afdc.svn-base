<?php
    ...
    foreach($listview_entries as $index=>$entry_info){
        $profilename=getGivenNamesByids($entry_info['profileid']);
        $listview_entries[$index]["profileid"]='<a href="javascript:;" data-toggle="lookupback" data-args="{id:\''.$index.'\',name:\''.$entry_info['profileid'].'\'}" title="'.$entry_info['profileid'].'">'.$profilename.'</a>';
    }
