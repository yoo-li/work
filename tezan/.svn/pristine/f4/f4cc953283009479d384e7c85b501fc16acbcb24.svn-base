  ...
  Selectpicker.prototype = {
    ...
    setSize: function () {
      ...
      if (this.options.size == 'auto') {
        ...
        /*K'naan add -- BEGIN*/
        var $lis       = menuInner.find('> li');
        var realHeight = parseFloat($lis.outerHeight() * $lis.length);
        var maxHeight  = parseFloat(menu.css('max-height'));
        if (!menu.data('add20') && realHeight > maxHeight) {
          menu.css('width', menu.width() + 20).data('add20', true);
        }
        /*K'naan add -- END*/
