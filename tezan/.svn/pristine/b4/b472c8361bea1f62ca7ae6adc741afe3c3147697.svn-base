    ...
    isset($_REQUEST['type']) && $_REQUEST['type'] == "submit" )
{
    try {
		...
		$loadcontent->save("Mall_RobSingles_details,Mall_RobSingles_details_".$supplierid);
		
		
	    $activity_products=XN_Query::create("Content")
	        ->tag("Mall_RobSingles_details_".$supplierid)
	    ...
	    foreach($activity_products as $apinfo){
	        ...
	        $apinfo->save("Mall_RobSingles,Mall_RobSingles_details".$supplierid);
	    }
