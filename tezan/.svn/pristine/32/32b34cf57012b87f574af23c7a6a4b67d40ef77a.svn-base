function printlogisticstatistics(logisticpackageid)
{
	jQuery.pdialog.open("index.php?module=Mall_SettlementOrders&action=PrintLogisticStatistics&oper=view&logisticpackageid="+logisticpackageid,"PrintLogisticStatistics","打印物流揽件对账单",{width:800,height:600});
}



function settlementorders_invoiceprint(record)
{
	if (selectprinter != "" && selectprinter != undefined)
	{
		var urlstring = 'module=Mall_SettlementOrders&action=MyInvoicePrint&record='+record;
