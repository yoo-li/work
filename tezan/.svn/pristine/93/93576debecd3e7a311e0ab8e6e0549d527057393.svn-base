...
if (!class_exists('vt_DateTime')) 
{
	class vt_DateTime
	{
		...
		{
			global $mod_strings;
			return $mod_strings['cal_month_short'][$this->month].$mod_strings['long_day'][$this->day];
		}
		...
		function get_formatted_datetime(){
			return $this->year."-".$this->z_month."-".$this->z_day." ".$this->z_hour.":".$this->minute.":".$this->second;
		}
		/**
		 ...
		 */
		function get_changed_hour($mode,$count=1){
			if($mode == 'increment')
				$hour = $this->hour + $count;
			else
				$hour = $this->hour - $count;
			
			$date_data = array('day'=>$this->day,
								'month'=>$this->month,
								'year'=>$this->year,
								'hour'=>$hour,
								'min'=>$this->minute,
		 ...
		 * return vt_DateTime obj
		 */
		function get_changed_day($mode,$count=1){
			if($mode == 'increment')
			$day = $this->day + $count;
			else
			$day = $this->day - $count;
		 ...
		 * return vt_DateTime obj
		 */
		function get_first_day_of_changed_month($mode,$arr=''){
		...
		function get_changed_year($mode,$count=1){
			if($mode == 'increment'){
		...
		function get_changed_month($mode,$count=1){
			if($mode == 'increment'){
