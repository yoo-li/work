...
(function($) {
	...
	$.Poshytip = function(elm, options) {
		...
		this.$tip = $(['<div class="',this.opts.className,'">',
			...
			'</div>'].join('')).appendTo($.CurrentNavtab);
		...
		display: function(hide) {
			...
			if ((this.opts.slide && this.pos.arrow || this.opts.fade) && (hide && this.opts.hideAniDuration || !hide && this.opts.showAniDuration)) {
				...
				if (this.opts.slide && this.pos.arrow) {
					...
					var pos_left = val + (hide ? 0 : (this.pos.arrow == arr ? -this.opts.slideOffset : this.opts.slideOffset));
					var pos_top  = val + (hide ? (this.pos.arrow == arr ? this.opts.slideOffset : -this.opts.slideOffset) : 0);
					if (pos_left <= 0 || pos_top <= 0)
					{ 
						this.$tip.data('active', false);
						return;
					} 
					else
					{
